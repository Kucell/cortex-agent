...
// ─── Redacted receipt ────────────────────────────────────────────────────────
// Builds a public-facing receipt that re-scans message and evidence for
// sensitive patterns. Sensitive content is rejected (only rule IDs returned)
// rather than included. The receipt is a summary — NOT a security boundary.
...
function buildRedactedReceipt(event, result) {
  ...
  // Scan message for sensitive content — include only if clean
  if (source.message) {
    const msgScan = scanContent(JSON.stringify(source.message));
    if (msgScan.length === 0) {
      receipt.message = source.message;
    } else {
      const redacted = [...(receipt.redactedFields || [])];
      redacted.push("message");
      receipt.redactedFields = redacted;
    }
  }

  // Scan evidence refs — include only clean refs, or redact the field
  if (source.evidence && source.evidence.length > 0) {
    const safeEvidence = source.evidence.filter((ev) => {
      const evScan = scanContent(JSON.stringify(ev));
      return evScan.length === 0;
    });
    if (safeEvidence.length > 0) {
      receipt.evidence = safeEvidence.map((ev) => ({ kind: ev.kind, ref: ev.ref }));
    } else {
      const redacted = [...(receipt.redactedFields || [])];
      redacted.push("evidence");
      receipt.redactedFields = redacted;
    }
  }
...
//   - taskId, projectId, coordinatorId, launchId
//   - producer info (actorId, kind, sessionId)
//   - repository, ownership scopes, target
//
// Idempotency: buildRetryDedupKey(launchId, eventType, deliveryId) ensures
// hooks that retry the same event type do not create duplicate submissions.

function createAgentReporterFromContext(service) {
  ...
  const actorId = assertNonEmptyString(context.coordinatorId, "coordinatorId");
  const projectId = assertNonEmptyString(context.projectId, "projectId");
  const sessionId = context.coordinatorId;

  // Extract taskId from the context for scoping
  const contextTaskId = context.taskId;
  ...
  function report(eventType, input) {
    ...
    const taskId = input.taskId || contextTaskId;
    if (!taskId) {
      return {
        ok: false,
    ...
    // Hook retry idempotency: if the same launchId + eventType + deliveryId
    // was already submitted, skip it.
    const deliveryId = input.deliveryId || null;
    const dedupKey = buildRetryDedupKey(context.launchId, eventType, deliveryId);
    if (dedupKey && dedupSet.has(dedupKey)) {
      return {
        ok: false,
    ...
    if (dedupKey) dedupSet.add(dedupKey);
