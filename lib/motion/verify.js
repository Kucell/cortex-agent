...
/**
 * Pixel-format family match. HyperFrames renders ProRes MOVs through its
 * alpha-capable path, so a preset asking for e.g. `yuv422p10le` may come back
 * as `yuva444p10le`/`yuva444p12le` — same ProRes family, still edits cleanly
 * in FCP/Resolve. We accept:
 *   - exact equality
 *   - any ProRes-family pix_fmt (both sides contain "444"/"422" + "10le"/"12le")
 *   - any yuv/rgb/yuva variant for the same bit depth family
 * Alpha presence is enforced separately by the `alpha` check (preset.alpha).
 * Pure + testable.
 */
function pixelFormatMatches(preset, actual) {
  if (!actual) return false;
  if (actual === preset.pixel_format) return true;
  // ProRes family: accept 422/4444 and 10/12-bit variants of each other.
  if (preset.codec === "prores_ks") {
    return /^(yu?a?va?|yuv|rgb)/i.test(actual) && /(10|12)le$/i.test(actual);
  }
  return false;
}
...
function validateRender(info, preset) {
  ...
  if (preset.pixel_format) {
    add(
      "pixel_format",
      pixelFormatMatches(preset, info.pixelFormat || ""),
 ...
 *   "npx" — `npx hyperframes lint|check` run inside the composition dir
 *            (hyperframes treats lint/check as project-level commands, so
 *            `cwd` must be the composition dir and no --motion-id is passed)
...
/**
 * Run `npx hyperframes lint|check` inside the composition dir (project-level).
 *   mode   — "lint" | "check"
...
function runCompositionCheck(options) {
  ...
  const entry = {
    ok: true,
    mode,
    message: `running hyperframes ${mode} in ${compDir}`,
    child,
  };
