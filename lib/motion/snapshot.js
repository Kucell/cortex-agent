...
// 命令(或 `npx hyperframes snapshot`,cwd=composition 目录)调用。
//
...
function captureSnapshot(options) {
  ...
  // hyperframes snapshot: project-level command — cwd 必须是 composition 目录;
  // -o/--output 接收的是输出目录(默认 <project>/snapshots),不是文件路径。
  const args =
    tool.tool === "npx"
      ? [...tool.args, "-o", snapshotsDir]
      : [...tool.args, "--composition-dir", compDir, "--output", out];
  const spawnFn = options.spawnFn || spawn;
  const child = spawnFn(tool.tool, args, {
    cwd: tool.tool === "npx" ? compDir : cwd,
