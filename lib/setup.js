...
function ensureProjectionRegistry(_ctx) {
  // The projection registry is shipped as a managed L1 file
  // (.agent/skills/management-api/scripts/projection-registry.json) and
  // is installed alongside the management-api skill. Keeping the
  // function no-op here preserves the upgrade API surface without
  // requiring a full registry merge implementation in the CLI path;
  // semantic merging for the registry is owned by the management-api
  // update flow, not by cortex-agent upgrade.
  return false;
}
