...
function mergeHookConfig(existing, incoming) {
  // Semantic merge: take incoming's matcher/event/payload shape and
  // merge with existing entries by event name. For each event, dedupe
  // hooks by `command` so updating a hook does not duplicate it. The
  // current production case only needs a stable JSON-shape contract;
  // advanced ordering / precedence is intentionally left to the
  // upstream caller.
  ...
  for (const [matcher, hooks] of Object.entries(incoming || {})) {
    ...
    for (const hook of result[matcher]) {
      if (hook && hook.command) byCommand.set(hook.command, hook);
    }
    for (const hook of list) {
      if (hook && hook.command) byCommand.set(hook.command, hook);
    }
    result[matcher] = Array.from(byCommand.values());
  }
  return result;
}
