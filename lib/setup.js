...
function ensureClaudeSettings(ctx) {
  ...
  const result = mergeHooks(settings.hooks, incomingHooks);
  if (!result.changed && fs.existsSync(settingsPath)) {
  ...
  console.log("✅ Updated .claude/settings.json with hook configuration.");
  return true;
}
