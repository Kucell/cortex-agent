...
function discoverTemplateScripts(templateDir) {
  const root = templateAgentDir(templateDir);
  const results = [];
  ...
  const walk = (dir) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        results.push(toRelative(root, abs));
      }
    }
  };
...
function ensureManifestForInit(cwd, templateDir, lang) {
  const relPaths = discoverTemplateScripts(templateDir);
...
function reconcileScripts(options) {
  ...
  const templateRoot = templateAgentDir(templateDir);
  const agentDir = projectAgentDir(cwd);
