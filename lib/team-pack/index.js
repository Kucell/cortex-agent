...
const secretScan = require("../security/scan");

const SCHEMA_VERSION = 1;
const SCHEMA_VERSION_V2 = 2; // P-002: file-level project scope
...
function manifestFingerprint(files) {
  ...
  const lines = sorted.map((f) => {
    const projects = Array.isArray(f.projects) && f.projects.length > 0
      ? `\t${[...f.projects].sort().join(",")}`
      : "";
    return `${f.path}\t${f.sha256}\t${f.mode}${projects}`;
  });
...
// P-002: validate a manifest file-level `projects` field (optional, v2).
// Returns an array of error strings (empty when valid).
function validateFileProjectsField(projects) {
  if (projects === undefined || projects === null) return [];
  if (!Array.isArray(projects)) return ["projects must be an array"];
  if (projects.length === 0) return [];
  if (projects.length > MAX_PROJECTS) return [`at most ${MAX_PROJECTS} project ids`];
  const seen = new Set();
  const errors = [];
  for (const id of projects) {
    if (typeof id !== "string" || !PROJECT_ID_RE.test(id) || id.length > 32) {
      errors.push(`invalid project id "${id}" (expected kebab-case [a-z0-9-], max 32)`);
      continue;
    }
    if (seen.has(id)) errors.push(`duplicate project id "${id}"`);
    seen.add(id);
  }
  return errors;
}

function hasProjectScopedFiles(manifest) {
  return Array.isArray(manifest.files) && manifest.files.some(
    (entry) => Array.isArray(entry.projects) && entry.projects.length > 0,
  );
}
...
function readManifest(projectRoot) {
  ...
  try {
    ...
    if (parsed.schema_version !== SCHEMA_VERSION && parsed.schema_version !== SCHEMA_VERSION_V2) return null;
    ...
    for (const entry of parsed.files) {
      ...
      if (validateFileProjectsField(entry.projects).length > 0) return null;
    }
    return parsed;
  } catch {
    return null;
...
function validateManifestShape(manifest, availableFiles) {
  const errors = [];
  if (manifest.schema_version !== SCHEMA_VERSION && manifest.schema_version !== SCHEMA_VERSION_V2) {
    errors.push(`unsupported schema_version=${manifest.schema_version}`);
  }
  if (manifest.schema_version === SCHEMA_VERSION && hasProjectScopedFiles(manifest)) {
    errors.push("files[].projects requires schema_version=2");
  }
  for (const declared of manifest.files) {
    const fieldErrors = validateFileProjectsField(declared.projects);
    if (fieldErrors.length) {
      errors.push(`files[${declared.path}].projects: ${fieldErrors.join("; ")}`);
    }
  }
...
// P-002: decide whether a file entry is visible to the selected agent.
// Returns { selected, reason, matched }.
function selectFileForAgent(file, agentProjects) {
  const projects = Array.isArray(file.projects) && file.projects.length > 0 ? file.projects : null;
  if (!projects) return { selected: true, reason: "global", matched: [] };
  const agentSet = new Set(agentProjects || []);
  const matched = projects.filter((id) => agentSet.has(id));
  if (matched.length > 0) {
    return { selected: true, reason: "project_intersection", matched };
  }
  return { selected: false, reason: "project_miss", matched: [] };
}

// Walk manifest.files and produce a plan against the current project state.
// `selection` = { agentId: string|null, agentProjects: string[] }. When the
// manifest carries project-scoped files, an explicit agent selection is
// required; otherwise we fail closed before producing any plan items.
function buildMergePlan(manifest, receipt, projectRootAbs, { dryRun = false, selection = null } = {}) {
  const hasScoped = hasProjectScopedFiles(manifest);
  if (hasScoped && !(selection && selection.agentId)) {
    const err = new Error(
      "buildMergePlan: project-scoped files present; --agent is required to select the active agent",
    );
    err.code = "ERR_AGENT_SELECTION_REQUIRED";
    throw err;
  }
  const agentProjects = selection && selection.agentId ? selection.agentProjects || [] : [];
  // VC-006: two applicable files mapping to the same target fail before writes.
  const seenTargets = new Map();
  for (const entry of manifest.files) {
    const scoped = selectFileForAgent(entry, agentProjects);
    if (!scoped.selected) continue;
    const target = entry.path.replace(/\\/g, "/");
    if (seenTargets.has(target)) {
      const err = new Error(
        `conflicting files map to the same target \"${target}\": \"${seenTargets.get(target)}\" and \"${entry.path}\"`,
      );
      err.code = "ERR_TEAM_PACK_DUPLICATE_TARGET";
      throw err;
    }
    seenTargets.set(target, entry.path);
  }
  const plan = {
    mode: dryRun ? "dry-run" : "apply",
    items: [],
    selection: {
      agent_id: selection && selection.agentId ? selection.agentId : null,
      scoped_files: hasScoped ? manifest.files.filter((f) => Array.isArray(f.projects) && f.projects.length > 0).length : 0,
    },
    selection_evidence: [],
  };
  ...
  for (const entry of manifest.files) {
    ...
    let decision;
    if (!scoped.selected) {
      decision = { action: "skip", reason: "project_miss", kept: null };
    } else {
    ...
    const item = {
      path: entry.path,
      mode: entry.mode,
      projects: Array.isArray(entry.projects) ? entry.projects.slice() : [],
      selected: scoped.selected,
      selection_agent_id: selection && selection.agentId ? selection.agentId : null,
      selection_reason: scoped.reason,
      base: baseSha,
      local: localSha,
    ...
    plan.selection_evidence.push({
      path: entry.path,
      projects: item.projects,
      selected: scoped.selected,
      selection_agent_id: item.selection_agent_id,
      baseline_sha256: baseSha,
      installed_sha256: scoped.selected ? installedSha : null,
      decision: decision.action,
...
function buildReceiptFromPlan(manifest, manifestSha256, plan, previousReceipt = null) {
  ...
  for (const item of plan.items) {
    if (item.decision === "apply" || item.decision === "add") {
      files.push({
        ...
        status: "installed",
        projects: item.projects && item.projects.length > 0 ? item.projects : undefined,
        selection_agent_id: item.selection_agent_id || undefined,
  ...
  // P-002: per-file selection evidence for every considered incoming file.
  if (Array.isArray(plan.selection_evidence) && plan.selection_evidence.length > 0) {
    receipt.selection_evidence = plan.selection_evidence.map((row) => ({
      path: row.path,
      projects: row.projects,
      selected: row.selected,
      selection_agent_id: row.selection_agent_id,
...
function loadPack(projectRootAbs) {
  ...
  const canonicalFiles = [...manifest.files]
    ...
    .map((f) => ({
      ...
      sha256: f.sha256,
      mode: f.mode,
      projects: Array.isArray(f.projects) && f.projects.length > 0 ? [...f.projects].sort() : undefined,
    }));
...
function verifyChecks(manifest, projectRootAbs, { strict = false } = {}) {
  ...
  const schemaOk = manifest.schema_version === SCHEMA_VERSION || manifest.schema_version === SCHEMA_VERSION_V2;
  out.checks.push({ id: "manifest_schema", status: schemaOk ? "pass" : "fail", reason: `schema_version=${manifest.schema_version}` });
  if (!schemaOk) out.ok = false;
...
module.exports = {
  ...
  validateFileProjectsField,
  hasProjectScopedFiles,
  selectFileForAgent,
  mergeDecision,
  buildMergePlan,
  emptyReceipt,
