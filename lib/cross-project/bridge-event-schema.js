...
// BRIDGE_EVENT_TYPES is the canonical allow-list of bridge event types that
// the producer side (bridge emit) and the consumer-side schema validator both
// accept. Additions require:
//
//   1. Updating this allow-list.
//   2. Updating tests/cross-project/bridge-event-schema.test.js the
//      "BRIDGE_EVENT_TYPES covers the four P-003 §4.3 event types" case.
//   3. Updating templates/{en,zh}/.agent/plans/proposals/projects/
//      cross-project-coordination/proposals/P-003-cross-project-event-bridge-proposal.md
//      §4.3 event-type table so the proposal reflects the new count.
//
// History:
//   • P-003 §4.3 (rc.1)   — initial 4: task.state_changed / decision.resolved /
//                            waitpoint.released / checkpoint.closed.
//   • P-008B §3.2 (rc.9)  — added mission.completed so consumer missions can
//                            emit reverse-direction completion events back to
//                            the producer (hmi-platform → SamHMI/M-019 loop).
