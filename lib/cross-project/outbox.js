...
// MS-003: Get outbox directory using shared runtime-layout API (VC-011)
// Uses new-first/legacy-fallback per VC-012 compatibility window
function outboxDirFor(root) {
  const crossProject = crossProjectDirFor(root);
  return path.join(crossProject, "outbox");
}

// MS-003: Get cross-project directory using shared runtime-layout API
