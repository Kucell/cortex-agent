...
function reportExitState(resultCode) {
  ...
  try {
    ...
    const service = CoordinationApplicationService.open(path.join(root, ".agent-runtime", "coordination"));
    ...
    if (task && !["READY_FOR_REVIEW", "COMPLETED", "FAILED", "BLOCKED", "CANCELLED"].includes(task.state)) {
      const eventType = resultCode === "EXIT_ZERO" ? "task.blocked" : "task.failed";
      const report = reporter.report(eventType, { message: resultCode === "EXIT_ZERO" ? "Agent exited without explicit handoff" : "Agent process exited abnormally", deliveryId: `child-exit:${context.launchId}` });
...
child.once("exit", (code, signal) => {
  ...
  const outcome = handoffReported ? "ALREADY_HANDED_OFF" : reportExitState(resultCode);
