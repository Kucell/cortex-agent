  ...
  try {
    ...
    if (task && !["READY_FOR_REVIEW", "COMPLETED", "FAILED", "BLOCKED", "CANCELLED"].includes(task.state)) {
      ...
      const report = reporter.report(eventType, {
        message: eventType === "task.blocked" ? "Agent exited or timed out without explicit handoff" : "Agent process exited abnormally",
        // Exit/timeout are coordinator-actionable facts. Do not inherit the
        // launch default (journal_only), otherwise a Pump correctly skips the
        // event and the coordinator can never recover the task.
        notificationPolicy: "coordinator_notify",
        deliveryId: `child-exit:${context.launchId}`,
      });
      outcome = report.ok ? (eventType === "task.blocked" ? "REPORTED_BLOCKED" : "REPORTED_FAILED") : "REPORT_REJECTED";
    } else outcome = "ALREADY_HANDED_OFF";
  } catch (error) {
    // Preserve only a stable category; private context and host errors must
    // never escape into a receipt.
    outcome = error && error.code === "ERR_LEASE_CONFLICT" ? "REPORT_REJECTED"
      : error && error.code === "ERR_INVALID_STATE" ? "TASK_READ_FAILED"
      : "SERVICE_OPEN_FAILED";
  }
