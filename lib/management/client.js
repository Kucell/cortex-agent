...
function managementScript(project, options = {}) {
  const existsSync = options.existsSync || fs.existsSync;
  const projectScript = path.join(project.agent_root, "skills", "management-api", "scripts", "index.js");
  if (existsSync(projectScript)) return projectScript;

  // Source worktrees do not carry the provider's complete nested .agent repo.
  // Restrict the packaged-runtime fallback to this cortex-agent checkout so a
  // normal target project with an incomplete .agent still fails closed.
  const packageRoot = options.packageRoot || canonicalDirectory(path.resolve(__dirname, "..", ".."));
  if (packageRoot && project.root === packageRoot) {
    return path.join(
      packageRoot,
      "templates",
      "_shared",
