...
// Backup / temp suffixes that the cortex-agent update flow leaves behind
// (e.g. index.js.bak, index.js.bak.prev when `--force-scripts` rewrites
// a managed script). They are NOT state and must never be staged. We
// filter them defensively here so state-sync is correct even if a user's
// .gitignore misses a pattern.
const STATE_PATH_BLOCKLIST = Object.freeze([
  /\.bak$/,
  /\.bak\.prev$/,
  /\.tmp$/,
  /~$/,
...
function isBackupPath(file) {
  for (const re of STATE_PATH_BLOCKLIST) {
...
// ─── stateSyncAuto — non-interactive entry for managementWrite etc. ──────────
//
// Used by `lib/commands.js` `managementWrite` and `init`/`upgrade` so the
// 9 state classes get add+commit+push'd automatically when a workflow
// writes to .agent/. Non-throwing: failures return { ok: false, error }
// and the caller decides whether to warn or fail.
//
// Escape hatch: CORTEX_STATE_SYNC=off.
//
// Returns one of:
//   { ok: true,  skipped: '<reason>' }                       // nothing to do
//   { ok: true,  summary: 'clean' }                           // working tree clean
//   { ok: true,  summary: 'committed <sha> locally' }        // no origin
//   { ok: true,  summary: 'pushed <sha> to origin/<branch>' } // happy path
//   { ok: false, error: '<stage>: <detail>' }                 // any failure
async function stateSyncAuto(ctx) {
  if (process.env.CORTEX_STATE_SYNC === "off") {
    return { ok: true, skipped: "CORTEX_STATE_SYNC=off" };
  }
  const agentDir = path.join(ctx.cwd, ".agent");
  if (!fs.existsSync(agentDir) || !fs.existsSync(path.join(agentDir, ".git"))) {
    return { ok: true, skipped: "no .agent/ git repo" };
  }
  const scan = scanState(agentDir);
  if (!scan.ok) return { ok: false, error: `scan: ${scan.error}` };
  if (scan.dirty.length === 0 && scan.staged.length === 0) {
    return { ok: true, summary: "clean" };
  }
  const addRes = addState(agentDir);
  if (!addRes.ok) return { ok: false, error: `add: ${addRes.error}` };
  const message = suggestCommitMessage(scan.dirty, scan.staged);
  const commitRes = commitState(agentDir, message);
  if (!commitRes.ok) {
    // "nothing to commit" is benign when --add staged nothing new.
    if (!/nothing to commit/i.test(commitRes.error || "")) {
      return { ok: false, error: `commit: ${commitRes.error}` };
    }
  }
  if (scan.remoteConfigured && scan.branch) {
    const pushRes = pushState(agentDir, scan.branch);
    if (!pushRes.ok) {
      return {
        ok: false,
        error: `push: ${pushRes.error}`,
        committedSha: commitRes.sha,
      };
    }
    return {
      ok: true,
      summary: `pushed ${commitRes.sha || "?"} to origin/${scan.branch}`,
      sha: commitRes.sha,
    };
  }
  return {
    ok: true,
    summary: `committed ${commitRes.sha || "?"} locally (no origin remote)`,
    sha: commitRes.sha,
  };
}

// ─── installStateGithooks — copy .githooks/ template + wire core.hooksPath ───
//
// Used by `bin/cli.js` after `case "init"` and `case "upgrade"`. Lives in
// lib/state-sync.js (not lib/commands.js) so M-001 shadow-init's invariant
// "lib/commands.js has 0 changes vs base" stays intact.
//
// Idempotent:
//   - existing .githooks/  → left untouched (user may have edited it)
//   - existing core.hooksPath pointing somewhere else → left alone + warn
//
// Returns { ok, installed, hooksConfigured, reason }.
function copyDirRecursive(src, dest) {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      } catch (_) { /* best effort: chmod is non-essential for tests */ }
    }
  }
}

function installStateGithooks(ctx) {
  const agentDir = path.join(ctx.cwd, ".agent");
  if (!fs.existsSync(agentDir)) {
    return { ok: false, installed: false, hooksConfigured: false, reason: "no .agent/" };
  }
  // Note: lib/state-sync/ → ../../templates/_shared (was `..` before f7d4100 reorg).
  const srcHooks = path.join(__dirname, "..", "..", "templates", "_shared", ".agent", ".githooks");
  if (!fs.existsSync(srcHooks)) {
    return { ok: false, installed: false, hooksConfigured: false, reason: "no template" };
  }
  const dstHooks = path.join(agentDir, ".githooks");
  let installed = false;
  if (!fs.existsSync(dstHooks)) {
    copyDirRecursive(srcHooks, dstHooks);
    installed = true;
  }
  const agentGit = path.join(agentDir, ".git");
  if (!fs.existsSync(agentGit)) {
    return { ok: true, installed, hooksConfigured: false, reason: ".agent/ is not a git repo" };
  }
  const getRes = spawnSync("git", ["-C", agentDir, "config", "--get", "core.hooksPath"],
    { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
  const current = getRes.status === 0 ? getRes.stdout.trim() : null;
  if (current === ".githooks") {
    return { ok: true, installed, hooksConfigured: true };
  }
  if (current && current !== ".githooks") {
    return {
      ...
      reason: `existing core.hooksPath=${current}`,
    };
  }
  const setRes = spawnSync("git", ["-C", agentDir, "config", "core.hooksPath", ".githooks"],
    { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
  if (setRes.status !== 0) {
    return {
      ok: false,
      installed,
      hooksConfigured: false,
      reason: `git config failed: ${(setRes.stderr || "").trim()}`,
    };
  }
  return { ok: true, installed, hooksConfigured: true };
}

// ─── fireAndForgetSync — wired from bin/cli.js for managementWrite tail ──────
//
// Non-throwing: any failure is logged via console.warn so the originating
// management write (decision / waitpoint / inbox / run / session / queue /
// task / event) still succeeds.
//
// Escape hatch: CORTEX_STATE_SYNC=off.
async function fireAndForgetSync(ctx) {
  if (process.env.CORTEX_STATE_SYNC === "off") return;
  try {
    const res = await stateSyncAuto({ cwd: ctx.cwd });
    if (!res.ok) {
      console.warn(`⚠️  state-sync auto: ${res.error}`);
      console.warn(`   Run \`cortex-agent state-sync --push --yes\` to retry.`);
    } else if (res.summary && res.summary !== "clean") {
      console.log(`🔄 state-sync: ${res.summary}`);
    }
  } catch (err) {
    console.warn(`⚠️  state-sync auto: ${err.message}`);
  }
}
