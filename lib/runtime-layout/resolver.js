...
// ─── Layout activation detection (MS-003) ────────────────────────────────────
//
// Detects whether the new runtime layout has been activated. The activation
// marker is `.agent/runtime/layout.json` (written last by the migration).
//
// Returns:
//   { activated: true }  — new layout is active; use new paths only
//   { activated: false, legacyFallback: true } — compat window; try new, fallback legacy
//   { activated: false, legacyFallback: false } — compat window, new path missing
//
// Consumers MUST:
//   - READ: try new path first; on ENOENT fall back to legacy (if exists)
//   - WRITE: only write to new path when activated; otherwise write to legacy
//
// Source: P-002 §4 兼容策略 / VC-012

const LAYOUT_ACTIVATION_MARKER = "layout.json";

function isNewLayoutActivated(root) {
  const projectRoot = assertProjectRoot(root);
  const markerPath = path.join(projectRoot, AGENT_DIR_SEGMENT, RUNTIME_DIR, LAYOUT_ACTIVATION_MARKER);
  return exists(markerPath);
}

function resolveRuntimePaths(projectRoot) {
  const root = assertProjectRoot(projectRoot);
  const activated = isNewLayoutActivated(root);
  const legacy = path.join(root, LEGACY_RUNTIME_SEGMENT);
  const newRuntime = path.join(root, AGENT_DIR_SEGMENT, RUNTIME_DIR);
  const legacyExists = exists(legacy);
  return {
    activated,
    legacyExists,
    legacyRuntimeDir: legacy,
    newRuntimeDir: newRuntime,
    // Helpers for common namespaces
    coordination: {
      new: path.join(newRuntime, "coordination"),
      legacy: path.join(legacy, "coordination"),
    },
    dispatch: {
      new: path.join(newRuntime, "dispatch"),
      legacy: path.join(legacy, "dispatch"),
    },
    "cross-project": {
      new: path.join(newRuntime, "cross-project"),
      legacy: path.join(legacy, "cross-project"),
    },
    handoffs: {
      new: path.join(root, AGENT_DIR_SEGMENT, HANDOFFS_DIR),
      legacy: path.join(legacy, "handoffs"),
    },
  };
}

/**
 * Read a file from the new path first, falling back to legacy during the
 * compatibility window. Returns { path, source } where source is 'new' or 'legacy'.
 * Throws on both-missing or corruption.
 */
function readWithLegacyFallback(readFn, newPath, legacyPath) {
  // Try new first
  try {
    const data = readFn(newPath);
    return { data, path: newPath, source: "new" };
  } catch (error) {
    if (!error || error.code !== "ENOENT") throw error;
  }
  // Fall back to legacy
  try {
    const data = readFn(legacyPath);
    return { data, path: legacyPath, source: "legacy" };
  } catch (error) {
    if (error && error.code !== "ENOENT") {
      const e = new Error(`readWithLegacyFallback: both paths failed; new=${newPath} legacy=${legacyPath}`);
      e.code = "LAYOUT_BOTH_MISSING";
      e.cause = error;
      throw e;
    }
    const e = new Error(`readWithLegacyFallback: file not found in new or legacy; new=${newPath} legacy=${legacyPath}`);
    e.code = "LAYOUT_NOT_FOUND";
    e.newPath = newPath;
    e.legacyPath = legacyPath;
    throw e;
  }
}

/**
 * Determine the write path based on activation state.
 * Returns { path, mode } where mode is 'new' or 'legacy'.
 * Throws if legacy doesn't exist during compat window.
 */
function resolveWritePath(newPath, legacyPath, projectRoot) {
  const activated = isNewLayoutActivated(projectRoot);
  if (activated) {
    return { path: newPath, mode: "new" };
  }
  // Compat window: write to legacy if it exists, otherwise create new
  if (exists(legacyPath)) {
    return { path: legacyPath, mode: "legacy" };
  }
  // Legacy doesn't exist: this shouldn't normally happen (migration creates it)
  // but we fall back to new for fresh installs
  return { path: newPath, mode: "new-fallback" };
}
...
module.exports = {
  ...
  exists,
  detectLegacyRuntime,
  // MS-003 consumer helpers
  isNewLayoutActivated,
  resolveRuntimePaths,
  readWithLegacyFallback,
  resolveWritePath,
  LAYOUT_ACTIVATION_MARKER,
};