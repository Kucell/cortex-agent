...
const PLATFORM_REGISTRY = {
  ...
  dsh: {
    ...
    desc: {
      zh: "DSH (DeepSeek Harness)：first-class dispatch adapter，settings.json + AGENTS.md + skills/workflows 符号链接",
      en: "DSH (DeepSeek Harness): first-class dispatch adapter with settings.json + AGENTS.md + skills/workflows symlinks",
    },
    files: [
      { src: "dsh/settings.json", dest: ".dsh/settings.json", merge: true },
      { src: "dsh/README.md", dest: ".dsh/README.md" },
      { src: "dsh/AGENTS.md", dest: ".dsh/AGENTS.md" },
    ],
    links: [
      { target: "../.agent/skills", link: ".dsh/skills" },
      { target: "../.agent/workflows", link: ".dsh/workflows" },
    ],
    cleanupPaths: [".dsh"],
  },
  qoderclicn: {
...
// `.agent-runtime` participates in the same track/untrack lifecycle as
// `.agent`: untracked by default (init writes it to .git/info/exclude),
// opt-in via `cortex-agent track` / `init --track`.
const BASE_PATHS = [".agent", ".agent-runtime", ".agents", "AGENTS.md", "GEMINI.md"];
