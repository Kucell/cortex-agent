...
async function runMemoryValidate(parsed, ctx) {
  ...
  if (quiet) {
    // hooks / CI: exit code only; one-line summary on stderr (stdout stays clean)
    if (!result.ok) {
      process.stderr.write(`memory validate: ${result.reason}\n`);
    } else if (result.issues.length > 0) {
      process.stderr.write(
        `memory validate: ${result.issues.length} issue(s) found (drift=${result.summary.drift}, missing=${result.summary.missing}, schema=${result.summary.schema}, orphan=${result.summary.orphan}, duplicate=${result.summary.duplicate}, over-cap=${result.summary["over-cap"]})\n`
      );
    }
    process.exitCode = result.ok ? (result.issues.length > 0 ? 1 : 0) : 1;
    return;
  }
  if (!result.ok) {
    process.stderr.write(`memory validate: ${result.reason}\n`);
