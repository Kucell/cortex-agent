...
function memoryCommand(ctx) {
  ...
  if (parsed.showHelp || !parsed.subcommand) {
    ...
    if (!parsed.subcommand && !parsed.showHelp) {
      process.stderr.write("Error: subcommand required (recall|distill)\n");
    }
    return;
  }
  ...
  // Unknown subcommand
  process.stderr.write(`Error: unknown memory subcommand "${parsed.subcommand}". Valid: recall, distill.\n`);
  printHelp();
  process.exitCode = 2;
}
