...
async function runSecretsAsync(ctx, dependencies) {
  ...
  if (action === "store") {
    // Public CLI is fail-closed on --value and on missing --from-env: we
    // never want a credential to enter argv (which would show up in shell
    // history and `ps` while the child is starting).  We spawn the skill
    // with `--from-env` plus the env-var name only; the secret flows via
    // the child process environment, never via command line.
    ...
    // Forward process.env so the child reads the secret via its inherited
    // environment, never via argv.  We use a *snapshot* (not a reference)
    // so that subsequent mutations of process.env (e.g. a test teardown
    // `delete process.env.X`) cannot retroactively alter the captured
    // env-var in the spawn options — which would otherwise make the
    // "no value in argv" assertion depend on test ordering.
    const child = spawn(process.execPath, [
      script, "store", "--ref", ref, "--from-env", envName, "--gate", "user",
    ], { cwd: ctx.cwd, encoding: "utf8", env: { ...process.env } });
    const body = parseJson(child.stdout);
    return child.status === 0 && body && body.ok
      ? result(true, { action, ref, secret_uri: `secret://${ref}`, backend: "configured", value_source: body.value_source || "env" })
      : result(false, { error: "secrets_backend_failed", action, ref, detail: body }, child.status || 1);
  }
