...
class Journal {
  ...
  *_iterateRecords() {
  ...
  _acquireLock() {
    ...
    const lockPayload = () => stableStringify({
      owner,
      token,
      acquiredAt,
      expiresAt: new Date(Date.now() + this._lockTtlMs).toISOString(),
      pid: process.pid,
    });
    ...
    try {
      ...
      try {
        ...
        if (!moved || moved.owner !== (existing && existing.owner)) {
          try { fs.renameSync(stalePath, lockPath); } catch { /* fail closed below */ }
          throw new CoordinationError("ERR_LEASE_CONFLICT", {
            details: { resource: "journal", lockFile: lockPath, reason: "reclaim_raced" },
          });
        }
        // A live owner always wins. A dead owner is reclaimable immediately,
        // even when its TTL has not elapsed: after a crash, waiting up to the
        // full lease duration prevents watchdogs and notification recovery.
        if (moved && isProcessAlive(moved.pid)) {
          try { fs.renameSync(stalePath, lockPath); } catch { /* fail closed below */ }
          throw new CoordinationError("ERR_LEASE_CONFLICT", {
            details: { resource: "journal", lockFile: lockPath, reason: "lock_renewed" },
          });
        }
        lockFd = tryCreate();
      } catch (err2) {
    ...
    this._lockFd = lockFd;
    this._lockAcquiredAt = acquiredAt;
    this._lockToken = token;
    this._lockFailure = null;
    const renewalMs = Math.max(1, Math.floor(this._lockTtlMs / 3));
    this._lockTimer = setInterval(() => this._renewLock(), renewalMs);
  ...
  _renewLock() {
    ...
    try {
      ...
      while (offset < data.length) {
        ...
        if (written <= 0) throw new Error("lock renewal write made no progress");
        offset += written;
      }
      if (this._fsync) fs.fsyncSync(this._lockFd);
    } catch (error) {
  ...
  _assertLockOwnership() {
    if (!this._lockEnabled) return;
    if (this._lockFailure) {
      throw new CoordinationError("ERR_LEASE_CONFLICT", {
        cause: this._lockFailure,
        details: { resource: "journal", reason: "lock_renewal_failed" },
      });
    }
    let lock;
    try {
      lock = JSON.parse(fs.readFileSync(this._lockPath, "utf8"));
    } catch (error) {
      throw new CoordinationError("ERR_LEASE_CONFLICT", {
        cause: error,
        details: { resource: "journal", reason: "lock_missing_or_corrupt" },
      });
    }
    if (lock.owner !== this._lockOwner || lock.token !== this._lockToken) {
      throw new CoordinationError("ERR_LEASE_CONFLICT", {
        details: { resource: "journal", reason: "lock_ownership_lost" },
      });
    }
  }
  ...
  _releaseLock() {
    ...
    try {
      ...
      if (lock && lock.owner === this._lockOwner && lock.token === this._lockToken) {
    ...
    if (this._lockFd != null) {
      try { fs.closeSync(this._lockFd); } catch { /* best effort */ }
      this._lockFd = null;
    }
    this._lockAcquiredAt = null;
    this._lockToken = null;
