...
function createCodexAdapter(host = {}) {
  const baseDescriptor = detectCodexCapabilities(host);
  const descriptor = attachV1CapabilityDescriptor(baseDescriptor, host);
  ...
  return Object.freeze({
    ...
    async deliver({ event }) {
      ...
      return {
        ...
        acknowledged: delivery.status === DELIVERY_RESULTS.DELIVERED,
