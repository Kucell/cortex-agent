...
class NotificationPump {
  ...
  async runOnce() {
    ...
    for (const key of pendingKeys) {
      ...
      if (!candidate) {
        ...
        report.deferred += 1;
        reasons.add("EVENT_NOT_VISIBLE");
        handled.add(key);
        continue;
      }
      const outcome = await this._attempt(candidate, key, report, reasons);
    ...
    for (const [key, candidate] of candidates) {
      ...
      await this._attempt(candidate, key, report, reasons);
    }

    this.cursor.advanceHighWater(events.length);
    for (const entry of Object.values(this.cursor.read().pending)) {
      if (entry.exhausted) exhausted += 1;
      if (entry.lastError) reasons.add(entry.lastError);
      else if (entry.exhausted) reasons.add("DELIVERY_EXHAUSTED");
    }
    if (report.deferred || report.failed || exhausted) {
  ...
  async _attempt({ event, target, policy }, key, report, reasons) {
    ...
    if (pending && (pending.exhausted || Date.parse(pending.nextAttemptAt) > this.clock())) {
      report.deferred += 1;
      if (pending.lastError) reasons.add(pending.lastError);
      else if (pending.exhausted) reasons.add("DELIVERY_EXHAUSTED");
      return true;
    }
    const outcome = await this._deliver(event, target, key, policy, pending, reasons);
    report[outcome] += 1;
    return true;
  }
  ...
  async _deliver(event, target, key, policy, previous, reasons) {
    ...
    try {
      ...
      if (result && result.status === "deferred") {
        const error = new Error(result.reason || "notification delivery deferred");
        error.code = result.errorCode || "ERR_NOTIFICATION_DEFERRED";
      ...
      reasons.add(String(error.code || error.message || "delivery_failed").slice(0, 256));
      this._recordRetry(event, target, key, attempts, error);
      return "failed";
    }
  }
