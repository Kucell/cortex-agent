...
class NotificationRuntime extends EventEmitter {
  constructor(options) {
    ...
    // A production watch is an explicitly started foreground service. Keep
    // both the watcher and fallback timer referenced so an idle journal does
    // not make the process exit before an event or stop request arrives.
    this.watchFactory = options.watchFactory || ((dir, listener) =>
      fs.watch(dir, { persistent: true }, listener));
  ...
  status() {
    const degraded = Boolean(this._lastError || (this._lastReport
      && (this._lastReport.degraded || this._lastReport.deferred || this._lastReport.failed)));
    return {
      state: this._state,
      consumerId: this.consumerId,
      cycles: this._cycles,
      ...
      degradedReasons: this._lastError
        ? [this._lastError.code || "ERR_NOTIFICATION_RUNTIME"]
        : (this._lastReport && this._lastReport.reasons) || [],
