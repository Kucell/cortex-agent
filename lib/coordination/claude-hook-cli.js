...
const HOOK_NOTIFICATION_POLICY = Object.freeze({
  ...
  PreToolUse: "deny",   // T-AGR-001: deny by default when not deferred
...
function executeClaudeHook(service, hookName, payload) {
  ...
  // ─── PreToolUse gate (T-AGR-001) ────────────────────────────────────────
  // Runs before tool execution. Enforces .agent path authorization for managed
  // worktrees. Returns a structured deny/allow without submitting events.
  //
  // If deferred = true: not a managed .agent path, let Claude decide.
  // If allowed = false: deny with a reason.
  //
  // This is the only hook that does NOT call the Agent Reporter — it returns
  // a structured response that the Claude Code harness uses to gate the tool.

  if (hookName === "PreToolUse") {
    // PreToolUse does NOT need the coordination service — it reads the grant
    // from CORTEX_LAUNCH_CONTEXT directly.
    const handlerResult = handlePreToolUse(payload);
    if (!handlerResult.ok) {
      return {
        ok: false,
        code: handlerResult.code || "ERR_PRETOOLUSE_FAILED",
        allowed: false,
        deferred: false,
        timestamp: new Date().toISOString(),
      };
    ...
    return {
      ok: true,
      allowed: handlerResult.allowed,
      deferred: Boolean(handlerResult.deferred),
