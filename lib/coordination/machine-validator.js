...
function optionalString(value, field, nullable = true) {
  if (value === undefined || (nullable && value === null)) return;
  if (typeof value !== "string" || value.length === 0) {
    invalid(`${field} must be a non-empty string${nullable ? " or null" : ""}`, field);
  }
}
...
function progress(value) {
  ...
  optionalString(value.phase, "progress.phase", false);
...
function requestedAction(value, kinds) {
  ...
  for (const name of ["ref", "decisionRef", "waitpointRef", "message"]) {
    optionalString(value[name], `requestedAction.${name}`, false);
  }
}
