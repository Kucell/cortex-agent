...
// ─── PreToolUse handler (T-AGR-001) ───────────────────────────────────────────
//
// Claude PreToolUse gate: enforces .agent path authorization for managed worktrees.
//
// Contract:
//   - Business code paths: defer to Claude default (allowed = true, deferred = true)
//   - Shared .agent paths without grant: deny (allowed = false)
//   - Path canonicalizes to nearest existing ancestor (anti-symlink / non-existent escape)
//   - Bash directly touching shared .agent: deny by default (runtime updates via CLI/API)
//   - Result never leaks absolute paths in public events
//
// This handler is stateless; it reads the grant from CORTEX_LAUNCH_CONTEXT
// and the worktree path from the environment. No writes.
...
const {
  ...
  buildAgentRootGrant,
  checkPreToolUse,
} = require("../governed/agent-root-grant");

// CORTEX_LAUNCH_CONTEXT is set by the governed launcher. The grant is read
// from the private context file, never from stdin or environment variables.
function loadAgentRootGrant() {
  const contextFile = process.env.CORTEX_LAUNCH_CONTEXT;
  if (!contextFile || typeof contextFile !== "string") return null;
  try {
    const fs = require("node:fs");
    const content = fs.readFileSync(contextFile, "utf8");
    const ctx = JSON.parse(content);
    return (ctx && ctx.agentRootGrant) ? ctx.agentRootGrant : null;
  } catch (_) {
    return null;
  }
}

// Derive the worktree root from CORTEX_LAUNCH_CONTEXT if available.
// The context file is in a private temp dir under the worktree's tmp.
function deriveWorktreeRoot() {
  const contextFile = process.env.CORTEX_LAUNCH_CONTEXT;
  if (!contextFile || typeof contextFile !== "string") return null;
  // Context file is at <tmpdir>/cortex-launch-<id>/context.json
  // Parent dir is the launch dir, which is under os.tmpdir().
  // Walk upward to find a directory that contains .agent.
  const fs = require("node:fs");
  let current = contextFile;
  for (let i = 0; i < 8; i++) {
    current = path.dirname(current);
    if (current === path.dirname(current)) break; // root
    try {
      const stat = fs.statSync(path.join(current, ".agent"));
...
function handlePreToolUse(payload) {
  if (!payload || typeof payload !== "object") {
    return { ok: false, code: "ERR_INVALID_PAYLOAD", allowed: false, deferred: false };
  }

  const toolName = typeof payload.toolName === "string" ? payload.toolName : null;
  if (!toolName) {
    return { ok: true, allowed: true, deferred: true };
  }

  // Only gate Write / Edit / MultiEdit / Read / Bash.
  const GOVERNED_TOOLS = new Set(["Write", "Edit", "MultiEdit", "Read", "Bash"]);
  if (!GOVERNED_TOOLS.has(toolName)) {
    return { ok: true, allowed: true, deferred: true };
  }

  // Load the private grant.
  const agentRootGrant = loadAgentRootGrant();
  const worktreeRoot = deriveWorktreeRoot();

  // Extract the file path from the payload.
  // For Write/Edit: tool_input.file_path (absolute) or tool (file path from tool call).
  // For Read: tool_input.file_path or path.
  // For Bash: tool_input.command is forwarded (token-level path detection
  // happens inside checkPreToolUse via scanBashCommandForAgentPath).
  let filePath = null;
  if (payload.tool_input && typeof payload.tool_input === "object") {
    filePath = payload.tool_input.file_path || payload.tool_input.path || null;
  }
  let bashCommand = null;
  if (toolName === "Bash" && payload.tool_input && typeof payload.tool_input.command === "string") {
    bashCommand = payload.tool_input.command;
  }

  const result = checkPreToolUse(toolName, filePath, bashCommand, agentRootGrant, worktreeRoot);
  return {
    ok: true,
