...
const { resolveRuntimePaths, resolveWritePath } = require("../runtime-layout");
const {
...
// MS-003: BINDINGS_DIR_NAME resolved via runtime-layout
const SCHEMA_VERSION = "1.0";
const BINDINGS_DIR_NAME = "bindings"; // subdirectory under coordination namespace
...
function assertRuntimeScoped(dir) {
  ...
  // MS-003: Accept paths under either new layout or legacy runtime.
...
function assertNoSymlinkAncestors(candidate) {
  ...
  // Also detect when the immediate parent of the runtime-root segment is a
  // symlink dir (e.g. `base/linked/.agent-runtime/coordination` whose
  // `base/linked` is a symlink). Walk once past the runtime segment so we
  // do not reject platform aliases.
  let cursor = path.dirname(resolved);
  const cursorParent = path.dirname(cursor);
  while (cursorParent !== cursor) {
    try {
      if (fs.lstatSync(cursor).isSymbolicLink()) {
        throw new LocalHostBindingError("ERR_BINDING_SCOPE", {
          reason: "symlink_in_path",
...
function safeResolve(rootDir, consumerId) {
  ...
  // symlink at any of the parents, redirecting the write outside runtime root.
...
// MS-003: Get bindings directory using shared runtime-layout API (VC-011)
// Returns { new: ..., legacy: ... } for read-with-fallback pattern
function getBindingsDirs(projectRoot) {
  const paths = resolveRuntimePaths(projectRoot);
  return {
    new: path.join(paths.coordination.new, BINDINGS_DIR_NAME),
    legacy: path.join(paths.coordination.legacy, BINDINGS_DIR_NAME),
  };
}
...
// (Moved below the class definition to avoid TDZ on `LocalHostBindingStore`.)
...
class LocalHostBindingStore {
  ...
  static forProject(projectRoot, projectId, options = {}) {
    const dirs = getBindingsDirs(projectRoot);
    // Use the new path for the store; legacy is used for reads via assertRuntimeScoped
    // The safeResolve function will accept either new or legacy path
    const rootDir = dirs.new;
    return new LocalHostBindingStore(rootDir, projectId, options);
  }
