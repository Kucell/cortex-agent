...
function scrubReceipt(obj, field = "$") {
  if (obj === null || obj === undefined) return obj;
  if (typeof obj !== "object") return assertSafeReceiptField(obj, field);
  if (Array.isArray(obj)) {
    return obj.map((entry, index) => scrubReceipt(entry, `${field}[${index}]`));
  }
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (seen.has(value)) {
      throw new LocalHostBindingError("ERR_BINDING_RECEIPT", { field: `${field}.${key}`, reason: "cycle" });
    }
    out[key] = scrubReceipt(value, `${field}.${key}`);
  }
  return out;
}
