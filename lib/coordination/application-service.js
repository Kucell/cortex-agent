...
function assertActorAuthorized(state, event, context = {}) {
  ...
  // Narrow exception: the coordinator who created/assigned the task may
  // submit task.failed while the task is still in ASSIGNED state (before
  // the agent has accepted). This covers governed launch failures where
  // the subprocess cannot be spawned and the agent never accepted.
  if (event.eventType === "task.failed" && state.state === "ASSIGNED") {
    return; // coordinator may fail a task before agent acceptance
  }
