...
function describeNotificationContract() {
  return {
    ...
    options: {
      "--interval-ms": { min: 1000, max: 3600000, default: 30000 },
...
async function executeNotificationCommand(args, harness = {}) {
  ...
  try {
    ...
    if (parsed.action === "watch") {
      ...
      const intervalMs = parsed.options.intervalMs || 30000;
      const stopFlag = { value: false };
      global.__cortexNotificationStopFlag = () => { stopFlag.value = true; };
      notify({ stage: "start", payload: { intervalMs } });
      try {
        while (!stopFlag.value) {
          await pump.runOnce();
          await new Promise((resolve) => {
            const timer = setTimeout(resolve, intervalMs);
            // Cooperative stop: poll the flag instead of sleeping the full interval.
        ...
        global.__cortexNotificationStopFlag = undefined;
      }
