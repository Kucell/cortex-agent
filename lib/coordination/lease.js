...
class LeaseManager {
  ...
  _createLease({ scope, owner, actorId, ttl, now, takeover, recoveredFrom, idempotencyKey }) {
    const lease = {
  ...
  acquire(scope, owner, opts = {}) {
    ...
    const idempotencyKey = typeof opts.idempotencyKey === "string" && opts.idempotencyKey.length > 0
      ? opts.idempotencyKey
      : null;

    // FAE-007 idempotency-key short-circuit: same scope + same owner + same
    // key + active lease → return existing lease (no new audit, no fencing bump).
    // Same key + different owner → ERR_LEASE_IDEMPOTENCY_MISMATCH + audit.
    if (idempotencyKey) {
      const existing = this.findLeaseByIdempotencyKey(idempotencyKey, { clock });
      if (existing) {
        if (existing.scope !== scope) {
          throw new CoordinationError("ERR_LEASE_IDEMPOTENCY_MISMATCH", {
            details: {
              idempotencyKey, scope, existingScope: existing.scope,
              reason: "idempotency key bound to a different scope",
            },
          });
        }
        if (existing.owner !== owner) {
          this._auditLog({
            eventType: "ownership.conflict", scope, leaseId: existing.leaseId, actor: actorId,
            fencingToken: existing.fencingToken, reason: "idempotency_key_owner_mismatch", evidence,
            details: { currentOwner: existing.owner, requestedOwner: owner, idempotencyKey, takeoverRequired: false },
          });
          throw new CoordinationError("ERR_LEASE_IDEMPOTENCY_MISMATCH", {
            details: {
              idempotencyKey, scope, currentOwner: existing.owner, requestedOwner: owner,
              leaseId: existing.leaseId,
            },
          });
        }
        // Same scope + same owner + same key. If the existing lease is still
        // active, return it (idempotent). If it has expired/released, fall
        // through to the standard acquire path below to issue a fresh lease.
        if (this._isActive(existing, now)) {
          return existing;
        }
      }
    }
    ...
    if (active) {
      if (active.owner === owner) {
        ...
        const renewed = this.renew(active.leaseId, { ttl, actorId: opts.actorId, clock, evidence });
        if (idempotencyKey) renewed.idempotencyKey = idempotencyKey;
        return renewed;
      }
  ...
  // FAE-007 read-only helper: full scope history (active + released + expired + stale).
  // Returns insertion-order leases for the scope; never throws on missing scope.
  listLeasesForScope(scope, opts = {}) {
    if (typeof scope !== "string" || scope.length === 0) {
      throw new CoordinationError("ERR_INVALID_STATE", {
        details: { reason: "scope required", scope },
      });
    }
    const clock = opts.clock || this._clock;
    const now = clock.now();
    const out = [];
    for (const lease of this._leasesForScope(scope)) {
      const snap = this._snapshot(lease);
      snap.status = this._leaseStatus(lease, now);
      snap.remaining_ms = this._remainingMs(lease, now);
      out.push(snap);
    }
    return out;
  }

  // FAE-007 read-only helper: lookup by leaseId with status + remaining_ms derived.
  findLeaseById(leaseId, opts = {}) {
    if (typeof leaseId !== "string" || leaseId.length === 0) {
      throw new CoordinationError("ERR_INVALID_STATE", {
        details: { reason: "leaseId required", leaseId },
      });
    }
    const clock = opts.clock || this._clock;
    const lease = this._leases.get(leaseId);
    ...
    const snap = this._snapshot(lease);
    snap.status = this._leaseStatus(lease, clock.now());
    snap.remaining_ms = this._remainingMs(lease, clock.now());
    return snap;
  }

  // FAE-007 read-only helper: idempotency-key lookup. Lease must carry
  // idempotencyKey field (set by acquire() when opts.idempotencyKey is provided).
  findLeaseByIdempotencyKey(idempotencyKey, opts = {}) {
    if (typeof idempotencyKey !== "string" || idempotencyKey.length === 0) {
      throw new CoordinationError("ERR_INVALID_STATE", {
        details: { reason: "idempotencyKey required", idempotencyKey },
      });
    }
    const clock = opts.clock || this._clock;
    for (const lease of this._leases.values()) {
  ...
  // FAE-007 status classifier (active / expired / released / stale / missing).
  _leaseStatus(lease, now) {
    if (!lease) return "missing";
    if (lease.releasedAt) return "released";
    if (lease.staleAt) return "stale";
    if (toMs(lease.expiresAt) <= now) return "expired";
    return "active";
  }

  _remainingMs(lease, now) {
    if (!lease) return 0;
    if (lease.releasedAt || lease.staleAt) return 0;
    const remaining = toMs(lease.expiresAt) - now;
  ...
  completeTakeover(requestId, opts = {}) {
    ...
    const ownerSessionId = opts.sessionId || req.requester;
    ...
    const lease = this._createLease({
      scope: req.scope, owner: req.requester, actorId: ownerSessionId, ttl, now,
      takeover: true, recoveredFrom: req.previousLeaseId, idempotencyKey: null,
    });
  ...
  exportState() {
    return {
      version: 1,
      leases: [...this._leases.values()].map((lease) => ({ ...lease })),
      fencing: Object.fromEntries(this._fencing),
      takeovers: [...this._takeovers.values()].map((request) => ({
        ...request,
      ...
      counters: {
        lease: this._leaseSeq,
        audit: this._auditSeq,
        takeover: this._tkoSeq,
      },
    };
  }

  static fromState(state, { clock } = {}) {
    if (!state || state.version !== 1 || !Array.isArray(state.leases)
        || !Array.isArray(state.takeovers) || !Array.isArray(state.audit)) {
      throw new CoordinationError("ERR_INVALID_STATE", {
        details: { reason: "invalid durable lease state" },
      });
    }
    const manager = new LeaseManager({ clock });
    for (const lease of state.leases) {
      if (!lease || !lease.leaseId || !lease.scope || !lease.owner
          || !Number.isInteger(lease.fencingToken)) {
        throw new CoordinationError("ERR_INVALID_STATE", {
          details: { reason: "invalid durable lease", leaseId: lease && lease.leaseId },
        });
      }
      manager._leases.set(lease.leaseId, { ...lease });
      manager._indexLease(lease.scope, lease.leaseId);
    }
    for (const [scope, token] of Object.entries(state.fencing || {})) {
      if (!Number.isInteger(token) || token < 0) {
        throw new CoordinationError("ERR_INVALID_STATE", {
          details: { reason: "invalid durable fencing token", scope },
        });
      }
      manager._fencing.set(scope, token);
    }
    for (const request of state.takeovers) {
      if (!request || !request.requestId || !request.scope || !request.requester) {
        throw new CoordinationError("ERR_INVALID_STATE", {
          details: { reason: "invalid durable takeover request" },
        });
      }
      manager._takeovers.set(request.requestId, { ...request });
      if (!manager._takeoversByScope.has(request.scope)) {
        manager._takeoversByScope.set(request.scope, []);
      }
      manager._takeoversByScope.get(request.scope).push(request.requestId);
    }
    manager._audit = state.audit.map((entry) => ({
      ...entry,
      evidence: [...(entry.evidence || [])],
      details: { ...(entry.details || {}) },
    }));
    const counters = state.counters || {};
    manager._leaseSeq = Number.isInteger(counters.lease) ? counters.lease : state.leases.length;
    manager._auditSeq = Number.isInteger(counters.audit) ? counters.audit : state.audit.length;
    manager._tkoSeq = Number.isInteger(counters.takeover) ? counters.takeover : state.takeovers.length;
    return manager;
  }
}
