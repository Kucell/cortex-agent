...
function exportAnchor(ctx) {
  ...
  // If `--project` wasn't passed, we honour `options.project` (top-level
  // --project that bin/cli.js lifts into ctx.options) but not anything else;
  ...
  try {
    ...
    // Hint to stderr so it doesn't pollute piped output (e.g. `... > CLAUDE.md`).
...
function printExportAnchorHelp(isZh) {
  console.log(
    isZh
      ? `Usage: cortex-agent export-anchor [options]

输出 cortex-agent 跨工具识别锚点片段，可粘贴到任何 AI 编码工具的长期记忆中。

Options:
  --format <markdown|json>   输出格式 (默认 markdown)
  --project <path>           项目根目录 (默认 cwd)
  --name <name>              覆盖项目名 (默认从 package.json / .git / 目录名推断)
  -h, --help                 显示此帮助

Examples:
  cortex-agent export-anchor                            # 输出 markdown 片段
  cortex-agent export-anchor --json                     # 输出 JSON 元数据
  cortex-agent export-anchor --project /path/to/proj    # 指定项目
  cortex-agent export-anchor > CLAUDE.md                # 写入 Claude Code 记忆
  cortex-agent export-anchor --json > .pi/anchor.json   # 写入 Pi agent`
      : `Usage: cortex-agent export-anchor [options]

Emit the cortex-agent cross-tool recognition anchor for this project.

Options:
  --format <markdown|json>   Output format (default markdown)
  --project <path>           Project root (default cwd)
  --name <name>              Override project name (auto-detected from package.json / .git / dir name)
  -h, --help                 Show this help

Examples:
  cortex-agent export-anchor                            # markdown snippet
  cortex-agent export-anchor --json                     # machine-readable JSON
  cortex-agent export-anchor --project /path/to/proj    # explicit project
  cortex-agent export-anchor > CLAUDE.md                # write into Claude Code memory
  cortex-agent export-anchor --json > .pi/anchor.json   # write into Pi agent`,
  );
}
...
async function doctor(ctx) {
  ...
  // Static-period check for the .agent/global symlink that linkGlobalConfig()
  // manages. Four classes: missing / not-symlink / broken / wrong-target.
  {
    const agentDir = path.join(cwd, ".agent");
    const globalLink = path.join(agentDir, "global");
    ...
    if (!fs.existsSync(globalLink)) {
      portKind = "missing";
      portDetail = `${globalLink} ${isZh ? "不存在" : "does not exist"}`;
    } else {
      let stat;
      try { stat = fs.lstatSync(globalLink); } catch (_) { stat = null; }
      if (!stat || !stat.isSymbolicLink()) {
        portKind = "not-symlink";
        portDetail = `${globalLink} ${isZh ? "不是 symlink" : "is not a symbolic link"}`;
      } else {
        let realTarget = null;
        try { realTarget = fs.realpathSync(globalLink); } catch (err) {
          portKind = "broken";
          portDetail = `${globalLink} ${isZh ? "断链" : "broken"}: ${err.message}`;
        }
        if (portKind === "ok" || portKind === "wrong-target") {
          let realHome = null;
          try { realHome = fs.realpathSync(homeAgent); } catch (err) {
            portKind = "home-missing";
            portDetail = `${homeAgent} ${isZh ? "不可解析" : "unresolvable"}: ${err.message}`;
          }
          if (portKind === "ok" && realTarget && realHome && realTarget !== realHome) {
            portKind = "wrong-target";
            portDetail = `${globalLink} → ${realTarget} (${isZh ? "期望" : "expected"} ${realHome})`;
          }
          if (portKind === "ok" && realTarget) {
            portDetail = `${globalLink} → ${realTarget}`;
          }
        }
      }
    }
    ...
    console.log(`\n[setup-portability]`);
    console.log(`  - ${isZh ? ".agent/global 状态" : ".agent/global status"}: ${labels[portKind] || portKind}`);
    console.log(`  - ${isZh ? "路径" : "path"}: ${portDetail}`);
    if (inGitRepo) {
      const tracked = hasTrackedPath(cwd, ".agent/global");
      console.log(`  - ${isZh ? "是否被 Git 跟踪" : "tracked by git"}: ${tracked ? (isZh ? "是" : "yes") : isZh ? "否" : "no"}`);
    }
    if (portKind === "broken" || portKind === "wrong-target" || portKind === "not-symlink") {
      console.log(`  - ${isZh ? "建议" : "remedy"}: ${isZh
        ? "删除 .agent/global 后重跑 `cortex-agent init`(会用相对路径重建)"
        : "remove .agent/global and re-run `cortex-agent init` (rebuilds with a relative target)"}`);
    } else if (portKind === "missing") {
      console.log(`  - ${isZh ? "建议" : "remedy"}: ${isZh
        ? "跑 `cortex-agent init` 创建相对路径 symlink"
        : "run `cortex-agent init` to create a relative symlink"}`);
    }
  }
