...
function upgrade(ctx) {
    ...
    for (const name of fs.readdirSync(srcDir)) {
      ...
      if (stat.isDirectory()) {
        ...
        if (sink.onMissingDir && !destExisted) {
          ...
          // sink returns false → we still need a usable `destPath` for any
          // nested `path.join` calls; use the source dir as a sentinel so
          // the inner recursion can no-op safely without writing anywhere.
          const ok = sink.onMissingDir(relPath, srcPath, destPath);
          if (!ok) {
            // Sentinel destPath: pass the source dir so recursive calls
            // can path.join without crashing; the sink is the only place
            // that touches a real file, and it never will for dry-run.
            walkWithSink(srcPath, srcPath, relPath, sink);
            continue;
          }
        }
  ...
  function collectReconcileCandidates() {
    let discovered = [];
    try {
      discovered = scriptManifest.discoverTemplateScripts(templateDir);
    } catch {
      return 0;
    }
    let count = 0;
    for (const relPath of discovered) {
      const srcAbs = path.join(templateDir, relPath.split("/").join(path.sep));
  ...
  if (dryRun) {
    ...
    if (candidateCount > 0) {
      console.log(isZh
        ? `\n📜 候选脚本更新 (${candidateCount})（加 --update-scripts 执行）：`
        : `\n📜 Candidate script updates (${candidateCount}) — run with --update-scripts to apply:`);
      console.log(isZh
        ? `   (--dry-run 不展开脚本级细节；试跑 \`cortex-agent upgrade\` 看完整列表。)`
        : `   (--dry-run does not enumerate script-level details; run \`cortex-agent upgrade\` for the full list.)`);
    }
    ...
    console.log(isZh
      ? "\nℹ️  重新执行 `cortex-agent upgrade` 应用上述新增；加 --update-scripts 同步管理 L1 脚本。"
      : "\nℹ️  Re-run `cortex-agent upgrade` to apply. Add --update-scripts to also refresh managed L1 scripts.");
    return;
  }

  // Apply path: existing behavior.
  ...
  // Reconcile managed L1 scripts: dry-run report by default; apply with --update-scripts.
  runScriptReconcile(ctx, {
    apply: options.updateScripts === true,
...
function printHelp() {
  ...
  console.log("  --update-scripts        (upgrade) Apply safe updates to managed L1 scripts");
  console.log("  --force-scripts         (upgrade) Also overwrite locally-modified managed scripts");
  console.log("  --dry-run               (upgrade) Report what would change; do not modify any file");
  console.log("  --fix                   (doctor) Apply fixes, incl. forced script updates");
  console.log("\nAvailable platforms:");
