...
function printHelp() {
  ...
  console.log("  update                  Add new files + safely refresh unmodified managed scripts");
  ...
  console.log("  query <projection>      Query a project Management API and output JSON");
  ...
  console.log("  --project <path>        Target an explicit project for management commands");
  console.log("  --since <date|time>     Inclusive activity query start boundary");
  console.log("  --until <date|time>     Inclusive activity query end boundary");
  ...
  console.log("  --update-scripts        (upgrade) Apply safe updates to managed L1 scripts (legacy alias)");
  console.log("  --force-scripts         (update) Also overwrite locally-modified managed scripts");
  console.log("  --dry-run               (upgrade/update) Report changes without modifying files");
