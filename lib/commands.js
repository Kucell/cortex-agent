...
function collectSemanticMergeCandidates(ctx) {
  ...
  if (false && needsProjectionRegistryMerge && needsProjectionRegistryMerge(ctx)) {
    candidates.push({
...
function runUpdateVerification(ctx, { full = false } = {}) {
  ...
  checks.push(managementQueryCheck(ctx, "capabilities"));
  checks.push(managementQueryCheck(ctx, "dashboard-state"));
  const today = new Date().toISOString().slice(0, 10);
  checks.push(managementQueryCheck(ctx, "activity", ["--since", today]));
  if (full) checks.push(managementQueryCheck(ctx, "activity"));
  ...
  if (options.team && !process.exitCode) {
    ...
    if (packLoaded.ok) {
      ...
      if (!dryRun) {
        ...
        const r2 = teamPack.buildReceiptFromPlan(packLoaded.manifest, packLoaded.manifestSha256, plan);
...
// publish / verify are reserved for MS-003 and rejected with a clear pointer.
...
function teamUsage(ctx) {
  ...
  switch (sub) {
    ...
    case "publish":
      return "Usage: cortex-agent team publish --paths <path...> (reserved for MS-003)";
    case "verify":
      return "Usage: cortex-agent team verify [--project <path>] [--strict] [--json] (reserved for MS-003)";
    default:
...
function applyPlanToProject(project, manifest, plan) {
  // Apply `add` / `apply` decisions. Conflicts are not written; caller records
  // them as conflicts in the receipt and conflict artifact.
  const applied = [];
  const conflicts = [];
  for (const it of plan.items) {
    ...
    } else if (it.decision === "conflict") {
      conflicts.push({ path: it.path, base: it.base, local: it.local, incoming: it.incoming });
    }
  }
...
async function teamInstall(ctx) {
  ...
  const receipt2 = teamPack.buildReceiptFromPlan(loaded.manifest, loaded.manifestSha256, plan);
...
function teamPublish(ctx) {
  console.error("❌ `team publish` is reserved for MS-003. See .agent/plans/proposals/projects/team-agent-pack/proposals/P-002-team-pack-cli-lifecycle-proposal.md §5.");
  process.exitCode = 3;
}

function teamVerify(ctx) {
  console.error("❌ `team verify` is reserved for MS-003. See .agent/plans/proposals/projects/team-agent-pack/proposals/P-002-team-pack-cli-lifecycle-proposal.md §4.");
  process.exitCode = 3;
}
...
function normalizePublishDest(source) {
  ...
  const top = segs[0];
  const ALLOW = new Set(["team-pack.json", "README.md", "rules", "workflows", "skills", "references", "schemas"]);
