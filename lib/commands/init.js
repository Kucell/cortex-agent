...
const { writePublicAnchor } = require("./anchor");
...
async function init(ctx) {
  ...
  // Skip global-shared-skills/ during the shared-layer copy: linkGlobalConfig
  // later creates a symlink from project .agent/global-shared-skills to the
  // Agent Skills standard namespace (~/.agents/skills/), so subsequent
  // cortex-agent update runs can flow vendor-skill upgrades through it
  // instead of keeping a frozen local copy.
  const initSharedExclude = new Set(["global-shared-skills"]);
  copyRecursive(sharedAgentSrc, agentDest, { exclude: initSharedExclude });
...
async function initModeGeneral() {
  ...
  // managed project. Mirrors what `init()` does for code mode (see patches.js).
  writeVersionFile(cwd);
  ...
  try {
    const { applyGitExclusion } = require("../platform/index");
