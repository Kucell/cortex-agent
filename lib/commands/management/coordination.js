...
function coordinationUsage(resource, action) {
  if (resource === "event") {
    return "Usage: cortex-agent event <list|ack> [options]";
  }
  const taskUsage = {
    create: "Usage: cortex-agent task create --project <path> --task <id> --actor <coordinator-id> --session <id> --project-id <id> --correlation-id <id> [--repository-id <id>] [--message <text>] [--notification-policy <policy>] [--auth-context-json <json>]",
    assign: "Usage: cortex-agent task assign --project <path> --task <id> --actor <coordinator-id> --session <id> --assignee <agent-id> [--assignee-kind <agent|adapter|service>] [--message <text>] [--auth-context-json <json>]",
    accept: "Usage: cortex-agent task accept --project <path> --task <id> --actor <assignee-id> --session <id> [--message <text>] [--auth-context-json <json>]",
  };
  if (taskUsage[action]) return taskUsage[action];
...
function coordination(ctx, dependencies = {}) {
  ...
  if (ctx.args.includes("--help") || ctx.args.includes("-h")) {
    const result = { ok: true, command: `${ctx.args[0]}.help`, usage: coordinationUsage(ctx.args[0], action) };
