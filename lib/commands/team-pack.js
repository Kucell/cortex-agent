...
function teamUsage(ctx) {
  ...
  switch (sub) {
    ...
    case "install":
      return "Usage: cortex-agent team install [--project <path>] [--agent <agent_id>] [--dry-run] [--report text|json]";
    case "update":
      return "Usage: cortex-agent team update [--project <path>] [--agent <agent_id>] [--dry-run] [--report text|json]";
    case "publish":
...
function resolveSelection(ctx, project, manifest) {
  const agentId = ctx.options && ctx.options.agent ? String(ctx.options.agent).trim() : "";
  if (!agentId) return null; // caller decides fail-closed semantics
  const entry = agentRegistry.readAgent(project, agentId);
  if (!entry) {
    const err = new Error(`team install: agent "${agentId}" not found in the agent registry`);
    err.code = "ERR_AGENT_NOT_FOUND";
    throw err;
  }
  return { agentId, agentProjects: agentRegistry.entryProjects(entry) };
}
...
async function teamInstall(ctx) {
  ...
  let selection = null;
  try {
    selection = resolveSelection(ctx, project, loaded.manifest);
  } catch (error) {
    console.error(`❌ ${error.message}`);
    process.exitCode = error.code === "ERR_AGENT_NOT_FOUND" ? 2 : 2;
    return;
  }
  const receipt = teamPack.readReceipt(project);
  let plan;
  try {
    plan = teamPack.buildMergePlan(loaded.manifest, receipt, project, { dryRun, selection });
  } catch (error) {
    if (error.code === "ERR_AGENT_SELECTION_REQUIRED") {
      console.error("❌ " + error.message);
      process.exitCode = 2;
      return;
    }
