...
function usage() {
  return [
    "Usage:",
    "  cortex-agent bridge sync [--source <id>] [--source-root <path>] [--auto] [--json]",
...
function bridgeSyncHandler(ctx, parsed) {
  ...
  const sourceRoot = parsed.flags.sourceRoot;
  if (!sourceRoot) {
    invalidUsage("--source-root <path> is required for `bridge sync` (Phase 1 does not auto-discover source projects)", parsed);
    return;
  }
...
function bridgeSubscribeHandler(ctx, parsed) {
  ...
  const source = parsed.flags.source;
  if (!source) {
    invalidUsage("--source <id> is required for `bridge subscribe`", parsed);
    return;
  }
