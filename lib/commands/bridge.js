...
function bridgeSyncHandler(ctx, parsed) {
  ...
  if (parsed.flags.auto) {
    // P-006 §3.5: --auto walks subscriptions + topology registry and
    // resolves host_root per source. No --source-root needed.
    const run = bridgeSync.syncFromTopology(targetRoot);
    const results = run.sources.map((entry) => entry.result);
    if (parsed.json) {
      ...
      for (const entry of run.sources) {
        ...
        if (!r.reachable) {
          console.log(`bridge sync (auto): source=${entry.source_project_id} status=unreachable errors=${r.errors.length}`);
          continue;
        }
        console.log(
          `bridge sync (auto): source=${entry.source_project_id} host=${entry.host_root} scanned=${r.scanned} matched=${r.matched} written=${r.written} skipped=${r.skipped} cursor=${r.cursor ? r.cursor.last_bridge_event_id || "<new>" : "<none>"}`,
        );
      }
      for (const u of run.unresolved) {
        console.error(`  ! unresolved source ${u.source_project_id}: ${u.reason}`);
      }
    }
    // Opportunistic: source-unreachable is exit 0 (P-003 §9.6). Other errors
    // bump to 3.
    const hasRuntimeError = run.sources.some((s) => s.result && s.result.ok === false && s.result.reachable);
    if (hasRuntimeError) process.exitCode = 3;
    return;
  }
  ...
  if (!sourceRoot) {
    invalidUsage("--source-root <path>, --topology-ref <peer>, or --auto is required for `bridge sync`", parsed);
    return;
  }
  const results = [];
  if (source) {
