...
function lease(ctx) {
  ...
  // M-035 MS-001: this declaration shadows the legacy one above, so the
  // --help early return must live here or `lease acquire --help` falls
  // through to lease-cli and fails with ERR_ARG_REQUIRED.
  if (ctx.args.includes("--help") || ctx.args.includes("-h")) {
    console.log(leaseUsage(ctx));
    return;
  }
