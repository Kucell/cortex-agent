...
// MS-002: Runtime layout migration planner and apply
const {
  buildMigrationPlan,
  applyMigration,
  buildMigrationReport,
} = require("./update/runtime-layout-migration");
...
function upgrade(ctx) {
  ...
  function walkWithSink(srcDir, destDir, relBase, sink, exclude) {
    if (!fs.existsSync(srcDir)) return;
    for (const name of fs.readdirSync(srcDir)) {
      // Skip excluded top-level segments entirely (no mkdir, no copy, no report).
      // Used to leave `.agent/global-shared-skills/` for linkGlobalConfig to
      // symlink to the Agent Skills standard namespace, so subsequent updates
      // flow vendor-skill upgrades through the symlink.
  ...
  // Skip global-shared-skills/ when copying the shared layer: it is
  // owned by linkGlobalConfig as a symlink to the Agent Skills standard
  // namespace (~/.agents/skills/), so subsequent update runs flow vendor
  // upgrades through the symlink instead of refreezing the local copy.
  const UPDATE_SHARED_EXCLUDE = new Set(["global-shared-skills"]);
  ...
  // MS-002: Build runtime layout migration plan (used by both dry-run and apply)
  let migrationPlan = null;
  let migrationReport = null;
  try {
    migrationPlan = buildMigrationPlan(ctx, { dryRun: true });
  } catch (error) {
    if (!reportJson) {
      console.warn(isZh
        ? `⚠️  无法构建运行时布局迁移计划: ${error.message}`
        : `⚠️  Could not build runtime layout migration plan: ${error.message}`);
    }
    migrationPlan = { is_noop: true, actions: [] };
  }

  if (dryRun) {
    // Shared base layer first, then language-specific overlay (language wins on conflict).
    walkWithSink(sharedAgentSrc, agentDest, "", reportSink, UPDATE_SHARED_EXCLUDE);
    ...
    // Include migration plan in dry-run report
    const dryRunWithMigration = {
      ...buildDryRunUpdateReport(ctx, {
        agentDest,
        wouldAdd,
        scriptCandidates,
        skippedChecks,
      ...
      migration: {
        ...
        layout_version: migrationPlan.layout_version,
        has_legacy: migrationPlan.has_legacy,
        already_migrated: migrationPlan.already_migrated,
        is_noop: migrationPlan.is_noop,
        actions: migrationPlan.actions.map((a) => ({
          id: a.id,
          type: a.type,
          ...
          verification_ids: a.verification_ids,
          phase: a.phase,
        })),
      },
    };
    ...
    if (reportJson) {
      printManagementPayload(dryRunWithMigration);
    ...
    // MS-002: Runtime layout migration dry-run output
    if (migrationPlan.has_legacy && !migrationPlan.is_noop) {
      const migrationActions = migrationPlan.actions.filter((a) => a.type !== "report_noop");
      console.log(isZh
        ? `\n🔀 运行时布局迁移计划 (${migrationActions.length} 操作):`
        : `\n🔀 Runtime layout migration plan (${migrationActions.length} actions):`);
      migrationActions.forEach((a) => {
        if (a.source_ref && a.target_ref) {
          console.log(isZh
            ? `   ${a.phase}: ${a.source_ref} → ${a.target_ref}`
            : `   ${a.phase}: ${a.source_ref} → ${a.target_ref}`);
        } else if (a.target_ref) {
          console.log(isZh
            ? `   ${a.phase}: ${a.target_ref} (${a.description})`
            : `   ${a.phase}: ${a.target_ref} (${a.description})`);
        }
      });
    } else if (migrationPlan.is_noop) {
      console.log(isZh
        ? `\n🔀 运行时布局: 已是最新 (无需迁移)`
        : `\n🔀 Runtime layout: already up to date (no migration needed)`);
    }
  ...
  walkWithSink(sharedAgentSrc, agentDest, "", applySink, UPDATE_SHARED_EXCLUDE);
  ...
  // MS-002: Apply runtime layout migration
  let migrationApplyResult = null;
  if (migrationPlan && migrationPlan.has_legacy && !migrationPlan.is_noop) {
    if (!reportJson) {
      console.log(isZh
        ? `\n🔀 执行运行时布局迁移...`
        : `\n🔀 Applying runtime layout migration...`);
    }
    try {
      migrationApplyResult = applyMigration(ctx, migrationPlan);
      migrationReport = buildMigrationReport(ctx, migrationPlan, migrationApplyResult);
      if (!reportJson) {
        if (migrationApplyResult.ok) {
          console.log(isZh
            ? `   ✓ 运行时布局迁移完成 (${migrationApplyResult.actions_applied.length} 操作)`
            : `   ✓ Runtime layout migration complete (${migrationApplyResult.actions_applied.length} actions)`);
        } else {
          console.error(isZh
            ? `   ✗ 运行时布局迁移失败: ${migrationApplyResult.errors.map((e) => e.error).join(", ")}`
            : `   ✗ Runtime layout migration failed: ${migrationApplyResult.errors.map((e) => e.error).join(", ")}`);
          if (migrationApplyResult.rollback) {
            console.log(isZh
              ? `   ↩  已回滚: ${migrationApplyResult.rollback.rolled_back.join(", ")}`
              : `   ↩  Rolled back: ${migrationApplyResult.rollback.rolled_back.join(", ")}`);
          }
        }
      }
    } catch (error) {
      if (!reportJson) {
        console.error(isZh
          ? `   ✗ 运行时布局迁移异常: ${error.message}`
          : `   ✗ Runtime layout migration exception: ${error.message}`);
      }
      migrationApplyResult = { ok: false, error: error.message };
      migrationReport = buildMigrationReport(ctx, migrationPlan, migrationApplyResult);
    }
  } else if (migrationPlan && migrationPlan.is_noop) {
    migrationReport = buildMigrationReport(ctx, migrationPlan, { ok: true, noop: true });
    if (!reportJson) {
      console.log(isZh
        ? `\n🔀 运行时布局: 已是最新 (跳过迁移)`
        : `\n🔀 Runtime layout: already up to date (migration skipped)`);
    }
  }
  ...
  ensureMemoryBootstrapEntry(ctx);
