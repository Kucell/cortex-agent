...
const { hasPiSettledSignal } = require("./pi-settlement");
...
const { buildClaudeAddDirs, supportsAddDirFlag } = require("./agent-root-grant");
...
function openService() {
  try {
    // MS-003: runtimeRoot is set from context.repository.worktreeId
    const paths = resolveRuntimePaths(runtimeRoot);
    const coordinationRoot = paths.activated || !paths.legacyExists
      ? paths.coordination.new
      : paths.coordination.legacy;
    return { ok: true, service: CoordinationApplicationService.open(coordinationRoot) };
  } catch (error) {
...
function reportFinal(resultCode) {
  ...
  try {
    ...
    const stdout = fileDigest(stdoutFile);
    const piSettled = resultCode === "EXIT_ZERO"
      && path.basename(context.agentCommand) === "pi"
      && hasPiSettledSignal(fs.readFileSync(stdoutFile, "utf8"));
    if (piSettled) {
      const report = reporter.report("task.ready_for_review", {
        message: "Governed Pi settled with an explicit host-protocol signal",
        evidence: [{ kind: "artifact", ref: `ARTIFACT-PI-${stdout.sha256.slice(0, 16)}` }],
        notificationPolicy: "coordinator_notify",
        deliveryId: `monitor-final:${context.launchId}:task.ready_for_review`,
      });
      if (!report.ok) return `FINAL_REJECTED_${report.code || "UNKNOWN"}`;
      return "REPORTED_READY_FOR_REVIEW";
    }
...
// T-AGR-001: Inject --add-dir= canonical .agent before the "--" prompt separator.
// The grant is private CORTEX_LAUNCH_CONTEXT only — never in public receipts.
// Host-aware (feedback pi-governed-launch-host-args): hosts that reject the
// Claude/Codex-only --add-dir flag (e.g. Pi) keep the private launch context /
// agentRootGrant via CORTEX_LAUNCH_CONTEXT but receive no --add-dir argv
// injection.
const addDirs = supportsAddDirFlag(context.targetAgentId)
  ? buildClaudeAddDirs(context.agentRootGrant || null)
  : [];
let childAgentArgs = context.agentArgs ? [...context.agentArgs] : [];
const sepIndex = childAgentArgs.indexOf("--");
if (sepIndex >= 0) {
  // Insert add-dirs right before the "--" separator.
  childAgentArgs = [...childAgentArgs.slice(0, sepIndex), ...addDirs, ...childAgentArgs.slice(sepIndex)];
} else if (addDirs.length > 0) {
  // No separator found: append add-dirs at the end.
  childAgentArgs = [...childAgentArgs, ...addDirs];
}

try {
  child = spawn(context.agentCommand, childAgentArgs, {
    cwd: runtimeRoot,
