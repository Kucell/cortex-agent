...
function supportsAddDirFlag(targetAgentId) {
  if (typeof targetAgentId !== "string") return false;
  const normalizedAgentId = targetAgentId.toLowerCase();
  for (const host of HOSTS_WITHOUT_ADD_DIR_FLAG) {
    if (normalizedAgentId === host || normalizedAgentId.startsWith(`${host}-`)) {
      return false;
    }
  }
  return true;
