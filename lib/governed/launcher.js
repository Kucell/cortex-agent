...
const { createEvent, STATES, createEventId } = require("../coordination/contract.js");
const { CoordinationError } = require("../coordination/errors.js");

const GOVERNED_LAUNCHER_SCHEMA_VERSION = "1.0";
const MAX_AGENT_ARGS = 64;
const MAX_AGENT_ARG_LENGTH = 4096;
...
//
// Canonical paths are used for identity and allowlist checks. Execution keeps
// the approved absolute path because multi-call shims (Volta, BusyBox, etc.)
// select behavior from argv[0] / the invoked basename.
...
function validateAgentCommand(command, options = {}) {
  ...
  const approvedPath = path.resolve(command);

  // Resolve a canonical identity — fail if the path does not exist.
  let resolved;
  try {
  ...
  try {
    if (resolved === fs.realpathSync(process.execPath)) {
      throw new GovernedLauncherError("ERR_AGENT_COMMAND_FALLBACK", {
        command: process.execPath,
        reason: "a symlink to process.execPath is not allowed as agentCommand",
      });
    }
  } catch (error) {
    if (error instanceof GovernedLauncherError) throw error;
  }
...
//   - Max 64 args
//   - Each arg is at most 4096 UTF-16 code units
...
function validateAgentArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.length > MAX_AGENT_ARG_LENGTH) {
      throw new GovernedLauncherError("ERR_AGENT_ARG_TOO_LONG", {
        index: i,
        max: MAX_AGENT_ARG_LENGTH,
      });
    }
    if (arg.includes("\0")) {
...
function createMonitorSpawnOptions(contextFile, privateContext) {
  return {
    cwd: privateContext && privateContext.repository && privateContext.repository.worktreeId
      ? privateContext.repository.worktreeId : undefined,
    stdio: "ignore",
    detached: true,
...
function defaultExecutor(contextFile, privateContext) {
  ...
  const child = spawn(
    process.execPath,
    [path.join(__dirname, "child-monitor.js"), contextFile, receiptFile],
    createMonitorSpawnOptions(contextFile, privateContext),
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      finish(() => {
        if (!fs.existsSync(agentSpawnedFile) || fs.existsSync(agentExitedFile)) {
          reject(new GovernedLauncherError("ERR_AGENT_EXITED_BEFORE_ACCEPTANCE", {}));
          return;
        }
    ...
    const probe = setInterval(() => {
      if (!fs.existsSync(agentExitedFile)) return;
      finish(() => reject(new GovernedLauncherError("ERR_AGENT_EXITED_BEFORE_ACCEPTANCE", {})));
    }, 25);
    probe.unref();

    child.once("error", (err) => {
      finish(() => reject(new GovernedLauncherError("ERR_EXECUTOR_FAILED", {
        reason: err.message,
      })));
    });

    child.once("exit", (code, signal) => {
      // A monitor exit before readiness means its child could not start.
      finish(() => reject(new GovernedLauncherError("ERR_EXECUTOR_EXITED_EARLY", {
        code,
        signal,
      })));
    });
...
function createPrivateLaunchContext(input) {
  ...
  // A fenced ownership lease is bound to the agent session, never to the
  // coordinator.  Direct library callers without a session retain the legacy
  // agent-id fallback; the public governed launcher always supplies one.
  const sessionId = assertOptionalString(input.sessionId, "sessionId") || targetAgentId;
  // leaseId and fencingToken are private governed-launch context fields.
  // They are passed by the coordinator via governed-launch-cli (CP-11) and are
  // NEVER exposed in the public launch result, event, or receipt.
  const leaseId = assertOptionalString(input.leaseId, "leaseId") || null;
  const fencingToken = (input.fencingToken !== null && input.fencingToken !== undefined)
  ...
  const notificationDelivery = input.notificationDelivery && typeof input.notificationDelivery === "object"
    ? Object.freeze({
      adapter: input.notificationDelivery.adapter,
      consumer: input.notificationDelivery.consumer,
      threadId: input.notificationDelivery.threadId,
    })
    : null;

  // T-AGR-001: Private agent-root grant for managed worktrees.
  // Contains canonicalAgentRoot and relative-path operation grants.
  // NEVER appears in public result, event, receipt, or journal.
  const agentRootGrant = (input.agentRootGrant && typeof input.agentRootGrant === "object")
    ? Object.freeze({
      schemaVersion: input.agentRootGrant.schemaVersion || "unknown",
      canonicalAgentRoot: input.agentRootGrant.canonicalAgentRoot || null,
      operations: input.agentRootGrant.operations && typeof input.agentRootGrant.operations === "object"
        ? Object.freeze({ ...input.agentRootGrant.operations })
        : Object.freeze({}),
    })
    : null;
  ...
  return Object.freeze({
    ...
    // Private governed fields: NOT exposed in public result/receipt.
    leaseId,
    fencingToken,
    taskId,
    projectId,
    ...
    launchId,
    coordinatorId,
    notificationTarget: Object.freeze({ actorId: coordinatorId, kind: "coordinator" }),
    agentCommand,
    ...
    launchedAt,
    // T-AGR-001: private — never in public result/event/receipt
    agentRootGrant,
  });
}
