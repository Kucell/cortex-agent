...
//
// T-AGR-001 integration:
//   - The launcher automatically detects managed worktrees (worktrees with a
//     canonical .agent containing rules/ and workflows/).
//   - A private agentRootGrant is built from the worktree's .agent root and
//     included in the private CORTEX_LAUNCH_CONTEXT.
//   - The canonical .agent is automatically added via --add-dir= before the
//     "--" prompt separator in the child command. No caller-supplied external
//     --add-dir values are permitted.
//   - The grant is private CORTEX_LAUNCH_CONTEXT only — it never appears in
//     public events, results, or receipts.
const fs = require("node:fs");
const path = require("node:path");
const { createEvent, STATES } = require("../coordination/contract.js");
const { validateThreadId } = require("../coordination/codex-app-server-client.js");
const {
...
const {
  buildAgentRootGrant,
} = require("./agent-root-grant");
const { observeAccepted } = require("./codex-p002-shadow-observer");
const { persistObservation } = require("./codex-shadow-persist");
...
async function executeGovernedLaunch(args, dependencies = {}) {
  ...
  const shadowAdmission = flag(args, "codex-shadow-admission");
  const requestedCodexNotification = args.includes("--notify-codex");
  const codexThreadId = flag(args, "codex-thread-id")
    || process.env.CORTEX_CODEX_THREAD_ID
  ...
  if (requestedCodexNotification && !codexThreadId) {
    return fail("ERR_CODEX_THREAD_ID_REQUIRED", "Codex notification requires --codex-thread-id or CORTEX_CODEX_THREAD_ID.", 4);
  }
  if (codexThreadId) {
    try { validateThreadId(codexThreadId); } catch (_) {
      return fail("ERR_CODEX_THREAD_ID", "Codex thread ID is invalid.", 4);
    }
  }
  ...
  // T-AGR-001: Detect managed project (canonical .agent with rules/ + workflows/).
  const agentRoot = path.join(worktree, ".agent");
  let canonicalAgentRoot;
  try {
    canonicalAgentRoot = fs.realpathSync(agentRoot);
  ...
  // T-AGR-001: Build private agentRootGrant only for managed projects.
  // Grant patterns are relative to the canonical .agent root.
  // The grant is private CORTEX_LAUNCH_CONTEXT only — never in public events/receipts.
  const agentRootGrant = buildAgentRootGrant(worktree, { grants: [] });
  ...
  const context = createPrivateLaunchContext({
    ...
    terminalTimeoutMs,
    notificationDelivery: codexThreadId ? {
      adapter: "codex",
      consumer: `governed-${task.createdBy}`,
      threadId: codexThreadId,
    } : null,
    // T-AGR-001: private agentRootGrant — only for managed projects; never in public events/receipts.
    agentRootGrant,
  });
  let contextFile;
  ...
  try {
    ...
    let shadowObserver;
    try {
      shadowObserver = (dependencies.shadowObserver || persistObservation)(shadowAdmission);
    } catch (_) {
      shadowObserver = { status: "failed", ok: false, error: "pilot_failed" };
    }
    return {
      ok: true,
      taskId,
