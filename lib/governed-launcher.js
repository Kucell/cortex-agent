...
function createPrivateLaunchContext(input) {
  ...
  // The agent reporter reads this producer from the context file and uses it
  // for all lifecycle events. The actorId is the real targetAgentId, not the
  // coordinatorId.
...
function createGovernedLauncher(service, options) {
  ...
  async function launch(input) {
    ...
    // ─── Sequence: create → assign → spawn → accept/fail ───────────────────
    // CRITICAL: create/assign MUST be submitted BEFORE spawn, so that if the
    // spawn completes immediately or the submit fails, the task already exists
    // in a known state for the failed event to reference.
    ...
    // Step 3: Write the private context to a temp file for the agent.
    const contextFile = writeContextFile(privateContext);

    // Track events and final state for the result
    const events = [
    ...
    try {
      // Step 4: AWAIT the executor — confirm the child process is alive
      // before submitting task.accepted. The executor Promise resolves only
      // after the child is confirmed alive (1000ms delay) or rejects on
      // spawn error / early exit.
      ...
      // The task was already created and assigned, so the failed event
      // references the correct previous state.
