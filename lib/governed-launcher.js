...
function defaultExecutor(contextFile, privateContext) {
  ...
  const child = spawn(process.execPath, [path.join(__dirname, "governed-child-monitor.js"), contextFile, receiptFile], {
    // `repository.worktreeId` was validated by the public launcher.  Binding
    // the child cwd here prevents a governed host from inheriting the
    // coordinator's unrelated shell directory.
    cwd: privateContext && privateContext.repository && privateContext.repository.worktreeId ? privateContext.repository.worktreeId : undefined,
    stdio: "ignore",
    detached: false,
    env: {
      ...process.env,
