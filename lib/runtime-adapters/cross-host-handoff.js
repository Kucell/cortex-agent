...
// P-002 Phase 2: extract cross-project metadata from a source object.
// All four fields are optional; missing/non-string/empty values fall back
// to null. Returned object is frozen so consumers can rely on the shape.
function extractCrossProjectMeta(source) {
  function pickStringOrNull(field) {
  ...
  return Object.freeze({
    origin_project: pickStringOrNull("origin_project"),
    target_project: pickStringOrNull("target_project"),
    topology_ref: pickStringOrNull("topology_ref"),
    working_branch: pickStringOrNull("working_branch"),
  });
}
...
function buildContextPackage(source) {
  ...
  // P-002 Phase 2: extract cross-project metadata (4 optional fields).
  // Each is string-or-null. Missing fields fall back to null so the output
  // shape is always stable (consumers don't have to check existence).
  const crossProjectMeta = extractCrossProjectMeta(source);
  ...
  // P-002 Phase 2: ensure 4 cross-project fields always present in
  // redacted_summary (override any pre-existing redacted value with the
  // resolved meta, which has already been null-coerced).
  redactedSource.origin_project = crossProjectMeta.origin_project;
  redactedSource.target_project = crossProjectMeta.target_project;
  redactedSource.topology_ref = crossProjectMeta.topology_ref;
  redactedSource.working_branch = crossProjectMeta.working_branch;
  ...
  const contextPackage = {
    ...
    cross_project: crossProjectMeta, // P-002 Phase 2
...
function handoff(source, requirement, snapshots, owner, options) {
  ...
  return Object.freeze({
    ...
    cross_project: contextPackage.cross_project, // P-002 Phase 2
  });
}
...
module.exports = {
  ...
  isTainted,
  redactValue,
  extractCrossProjectMeta, // P-002 Phase 2
};