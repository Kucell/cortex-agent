...
// extends the existing five-host list with `dsh` / `codey` / `minimax` so
// all seven P-007 hosts participate in the MiniMax CLI portable skill
// discovery surface (VC-031-002-04). Existing five hosts are preserved in
// stable order so existing call sites (CLI / tests / fixtures) see zero
// regression. HOST_PATH_BUILDER gains three host entries that follow the
// exact same `user / project / template` builder shape as the frozen
// entries — no new scopes, no new builder keys, no new enums.
const HOSTS = Object.freeze([
  ...
  "codex",
  "common",
  "dsh",      // P-007 / M-029 first-class
  "codey",    // P-007 / M-003 minimax sibling
  "minimax",  // P-007 / M-003 minimax (the "MiniMax" branded host)
]);
...
const HOST_PATH_BUILDER = Object.freeze({
  ...
  // P-007 §4.7 — three new builders mirroring the frozen shape. Each host
  // gets the canonical `~/.{host}/skills/minimax-cli` user root and the
  // project + template counterparts, all using the same SKILL_NAME so
  // enumerators remain canonical across the surface.
  dsh: {
    user: () => path.join(osHomedir(), ".dsh", "skills", SKILL_NAME),
    project: (projectRoot) => path.join(projectRoot, ".dsh", "skills", SKILL_NAME),
    template: (templatesRoot, locale) => path.join(templatesRoot, locale, ".dsh", "skills", SKILL_NAME),
  },
  codey: {
    user: () => path.join(osHomedir(), ".codey", "skills", SKILL_NAME),
    project: (projectRoot) => path.join(projectRoot, ".codey", "skills", SKILL_NAME),
    template: (templatesRoot, locale) => path.join(templatesRoot, locale, ".codey", "skills", SKILL_NAME),
  },
  minimax: {
    user: () => path.join(osHomedir(), ".minimax", "skills", SKILL_NAME),
    project: (projectRoot) => path.join(projectRoot, ".minimax", "skills", SKILL_NAME),
    template: (templatesRoot, locale) => path.join(templatesRoot, locale, ".minimax", "skills", SKILL_NAME),
  },
});
