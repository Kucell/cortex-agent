...
function buildAbsentDescriptor() {
  return {
    ...
    capabilities: {
      ...
      "subagent": { level: "unsupported", source: "not-exposed" },
      "prompt_guidance": { level: "unsupported", source: "not-exposed" },
    },
    // P-003 friction signal matrix (M-003A): absent host declares every
    // signal as not_supported and emits no lifecycle events. redaction is
    // "full" because an absent host cannot leak data either — the absent
    // descriptor is the most conservative possible posture.
    friction_signals: {
