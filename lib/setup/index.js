...
//     with a *relative* target computed from linkReal.
...
function ensurePortableSymlink(opts) {
  ...
  let linkNodeIsSymlink = false;
  try {
    linkNodeIsSymlink = fs.lstatSync(linkPath).isSymbolicLink();
  } catch (_) {
    linkNodeIsSymlink = false;
  }

  // 3. Existing link: if it already points at the canonical target, no-op.
  if (linkNodeIsSymlink) {
    try {
...
function linkGlobalConfig(ctx) {
  ...
  if (fs.existsSync(globalAgentsSkillsPath)) {
    ...
    const sharedLink = path.join(agentDir, "global-shared-skills");
    let sharedLinkExists = false;
    try {
      sharedLinkExists = fs.lstatSync(sharedLink).isSymbolicLink();
    } catch (_) {
      sharedLinkExists = false;
    }
    if (!sharedLinkExists) {
      try {
        // Use realpathSync for cross-directory compatibility (macOS /tmp -> /private/tmp)
        const realAgentDir = fs.realpathSync(agentDir);
        const realSharedPath = fs.realpathSync(globalAgentsSkillsPath);
        const relTarget = path.relative(realAgentDir, realSharedPath);
        const realSharedLink = path.join(realAgentDir, "global-shared-skills");
        fs.symlinkSync(relTarget, realSharedLink);
        recordCreated(".agent/global-shared-skills");
        console.log(isZh
          ? "✅ Linked .agent/global-shared-skills -> ~/.agents/skills"
          : "✅ Linked .agent/global-shared-skills -> ~/.agents/skills");
      } catch (err) {
        console.warn(`⚠️  Failed to create shared skills link: ${err.message}`);
      }
    }
  }
