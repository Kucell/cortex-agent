...
function mergedProjectionRegistry(ctx) {
  ...
  if (!current || !Array.isArray(current.projections)) return incoming;

  const incomingNames = new Set(incoming.projections.map((entry) => entry.name));
  return {
    ...current,
    ...incoming,
    projections: [
      ...incoming.projections,
      ...current.projections.filter((entry) => !incomingNames.has(entry.name)),
    ],
  };
}
