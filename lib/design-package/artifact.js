...
function buildArtifact(meta) {
  ...
  //         renderMode, views, scene, sourceProvider }
  const ts = nowIso();
  ...
  // MS-3: render-mode + views + scene provenance.
  if (meta.renderMode) metadata.renderMode = meta.renderMode;
  if (meta.sourceProvider) metadata.sourceProvider = meta.sourceProvider;
  if (Array.isArray(meta.views) && meta.views.length > 0) metadata.views = meta.views;
  if (meta.scene && typeof meta.scene === "object") metadata.scene = meta.scene;
