...
const VALID_RENDER_MODES = new Set(["summary", "board", "page", "dual"]);

const ENTRY_DEFAULT = "samhmi-editor.html";
const BOARD_ENTRY_SUFFIX = ".design-board.html";
...
function parseArgs(args, lang, cwd) {
  ...
  for (let i = 1; i < argv.length; i += 1) {
    ...
    } else if (a === "--render-mode" && argv[i + 1]) {
      opts.renderMode = argv[++i];
    } else if (a.startsWith("--render-mode=")) {
      opts.renderMode = a.slice("--render-mode=".length);
    } else if (a === "--page-id" && argv[i + 1]) {
      opts.pageId = argv[++i];
    } else if (a.startsWith("--page-id=")) {
      opts.pageId = a.slice("--page-id=".length);
    }
  }
  return opts;
}
...
function validateRenderMode(mode) {
  if (!VALID_RENDER_MODES.has(mode)) {
    throw new Error(
      `design-package: unknown --render-mode "${mode}". Valid: ${[...VALID_RENDER_MODES].join(", ")}`,
    );
  }
  return mode;
}

// Validate the combination of render-mode + page-id.
function validatePageRequirement(opts) {
  if (!opts) return null;
  const needsPage = opts.renderMode === "page" || opts.renderMode === "dual";
  if (needsPage && !opts.pageId) {
    throw new Error(`design-package: --render-mode ${opts.renderMode} requires --page-id <Pixso GUID>`);
  }
  return opts;
}

// Compute the on-disk entry filename for a given render-mode + page-id.
// - summary/board/page → opts.entry unchanged
// - dual → board is written next to opts.entry as `${base}.design-board.html`
function boardEntryName(entry) {
  if (typeof entry !== "string" || !entry.endsWith(".html")) return null;
  return entry.replace(/\.html$/, BOARD_ENTRY_SUFFIX);
}
...
function printHelp(isZh) {
  const text = isZh
    ? `\n用法:cortex-agent design-package <task-id> --from-pixso <dsl.json> [options]\n\n` +
      ...
      `  <entry>.design-board.html     仅 board / dual 模式:全量可缩放设计大画布\n` +
      ...
      `  --render-mode <id>        summary | board | page | dual(默认 summary)\n` +
      `  --page-id <guid>          page / dual 必填,必须是真实 1920×1080 节点\n` +
    ...
    : `\nUsage: cortex-agent design-package <task-id> --from-pixso <dsl.json> [options]\n\n` +
      ...
      `  <entry>.design-board.html     board / dual only: scalable full design board\n` +
      ...
      `  --render-mode <id>        summary | board | page | dual (default summary)\n` +
      `  --page-id <guid>          Required by page / dual; must be a 1920×1080 node\n` +
