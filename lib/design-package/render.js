...
function renderSamHmiHtml(brief, tokens, opts) {
  ...
  // Real source canvas from the Pixso DSL root; the HTML shell is a fixed
  // SamHMI editor viewport (1920×1080) and is labelled as such.
  const sourceCanvas = tokens.canvas || { w: 1920, h: 1080 };
  const sourceW = Math.round(Number(sourceCanvas.w) || 1920);
  const sourceH = Math.round(Number(sourceCanvas.h) || 1080);
      ...
      <div class="canvas">
        <div class="stage">
          <div class="stage-head">
            ...
            <span class="stage-sub">${escapeHtml(taskId)} · 源画布 ${sourceW}×${sourceH}</span>
          </div>
          <div class="stage-body">
${preview || `        <div class="card"><div class="card-title">标准模板控件详情</div><div class="demo-row">源画布 ${sourceW} × ${sourceH}</div></div>`}
          </div>
        </div>
      </div>
    </main>
    <aside class="panel" data-od-ref="${SHELL_REFS.inspector}">
      ...
      <div class="panel-body">
        <div class="section"><div class="sec-head">${escapeHtml(isZh ? "画面属性" : "Screen props")}</div>
          ...
          <div class="field"><span class="k">${escapeHtml(isZh ? "源画布" : "Source canvas")}</span><span class="v">${sourceW} × ${sourceH}</span></div>
          <div class="field"><span class="k">${escapeHtml(isZh ? "编辑器壳" : "Editor shell")}</span><span class="v">1920 × 1080</span></div>
        </div>
