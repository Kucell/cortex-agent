...
const VALID_SUBCOMMANDS = ["publish", "subscribe", "list-events", "history", "resume"];

// ---------------------------------------------------------------------------
// M-004 MS-003 / F-006c — `event-bus resume` subcommand
// ---------------------------------------------------------------------------
//
// parent-resume client CLI entry. Lists the current FSM state of all
// subscribed sub-agents for a given --mission-id, optionally with --force
// to retry an existing escalation decision (D-FAE-002-4).
//
// Strictly additive: this block does not modify any existing function body
// or validation rule. New argv flag: --force (boolean).
// ---------------------------------------------------------------------------

const parentResume = require("./clients/parent-resume");

function runResume(parsed) {
  if (!parsed.missionId) {
    process.stderr.write("event-bus resume: --mission-id is required\n");
    process.exitCode = 2;
    return;
  }

  const all = parentResume.listActive();
  const filtered = all.filter((a) => a.mission_id === parsed.missionId);

  let escalationRetry = null;
  if (parsed.force) {
    // --force: enumerate existing escalation decisions for this mission and
    // surface them for Eric to act on. This is a read-only list — the
    // actual re-resolution is the user's call (status flip in D-ESC-*.json).
    const decisionsDir = path.join(process.cwd(), ".agent", "decisions");
    let files = [];
    try {
    ...
    const matched = files.filter((f) => f.startsWith(`D-ESC-${parsed.missionId}-`) && f.endsWith(".json"));
    const decisions = matched.map((f) => {
      try {
        return JSON.parse(fs.readFileSync(path.join(decisionsDir, f), "utf8"));
      } catch (_) {
        return { decision_id: f, _unreadable: true };
      }
    });
    escalationRetry = {
      ok: true,
  ...
  const out = {
    ...
    action: "resume",
    mission_id: parsed.missionId,
    active_count: filtered.length,
    active: filtered,
    force: !!parsed.force,
    escalation_retry: escalationRetry,
  };
...
function parseArgs(args) {
  const out = {
    ...
    // resume (parent-resume client, M-004 MS-003)
    force: false,
  };
...
function printHuman(obj, parsed) {
  switch (obj.action) {
    ...
    case "history":
      ...
      break;
    case "resume":
      console.log(`mission_id:   ${obj.mission_id}`);
      console.log(`active_count:  ${obj.active_count}`);
      if (obj.active.length === 0) {
        console.log(`(no active sub-agents for this mission)`);
      } else {
        for (const a of obj.active) {
          console.log(`- ${a.subagent_id}  state=${a.state}  last_event_at=${a.last_event_at || "n/a"}`);
        }
      }
      if (obj.force && obj.escalation_retry) {
        console.log(`--force: ${obj.escalation_retry.decisions_found} escalation decision(s) for ${obj.mission_id}`);
        for (const d of obj.escalation_retry.decisions) {
          console.log(`  - ${d.decision_id || "(no id)"}  status=${d.status}  reason=${d.reason || ""}`);
        }
      }
      break;
    default:
...
function printHelp(lang) {
  ...
  if (zh) {
    ...
    console.log("");
    console.log("M-004 MS-003 扩展 (1):");
    console.log("  resume        parent-resume client: 列出 mission 的 active sub-agent FSM (--mission-id)");
    console.log("");
    ...
    console.log("");
    console.log("resume 必选 flag:");
    console.log("  --mission-id <id>     mission id (父 mission)");
    console.log("resume 可选 flag:");
    console.log("  --force               列出 .agent/decisions/D-ESC-<id>-*.json 等待 Eric 人工介入");
    console.log("");
    console.log("通用 flag:");
    ...
    console.log("");
    console.log("M-004 MS-003 extension (1):");
    console.log("  resume        parent-resume client: list mission active sub-agent FSM (--mission-id)");
    console.log("");
    ...
    console.log("");
    console.log("resume required flag:");
    console.log("  --mission-id <id>     mission id (parent mission)");
    console.log("resume optional flag:");
    console.log("  --force               list .agent/decisions/D-ESC-<id>-*.json awaiting Eric intervention");
    console.log("");
...
function eventBusCommand(ctx) {
  ...
  if (parsed.showHelp || !parsed.subcommand) {
    ...
    if (!parsed.subcommand && !parsed.showHelp) {
      process.stderr.write("Error: subcommand required (publish|subscribe|list-events|history|resume)\n");
    }
    return;
  }
  ...
  switch (parsed.subcommand) {
    ...
    case "resume":      return runResume(parsed);
  }
}
