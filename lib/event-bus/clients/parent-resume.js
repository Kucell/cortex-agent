...
// Default lease provider: read .agent-runtime/coordination/leases.json (best-effort)
function _defaultLeaseProvider() {
  return {
    isLeaseActive(missionId) {
      if (!missionId) return false;
      const file = path.join(_ensureRootDir(), ".agent-runtime", "coordination", "leases.json");
      let raw;
      try {
