...
function reconcileAttemptForReadiness(service, options = {}) {
  ...
  // Step 0: clear the stale Task.ownership reference. The previous lease has
  // already been released at the lease-store level. To keep the journal
  // authoritative without breaking the lease-check invariant (which is
  // protected by `assertActorAuthorized`), we call into the application
  // service's `__clearOwnershipForReconciler` privileged helper. That helper
  // is gated by an explicit opt-in flag in the auth context so that only
  // the fenced reconciler can exercise it.
  if (Array.isArray(task.ownership) && task.ownership.length > 0) {
    if (typeof service.__clearOwnershipForReconciler !== "function") {
      return { ok: false, code: "ERR_RECONCILER_HELPER_UNAVAILABLE" };
    }
    try {
      service.__clearOwnershipForReconciler(task.taskId, {
        actorId: lease.owner,
      ...
      return { ok: false, code: "ERR_OWNERSHIP_CLEAR_FAILED", message: clearError.message || "unable to clear stale ownership reference" };
    }
  }
