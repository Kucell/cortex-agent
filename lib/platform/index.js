...
const { PLATFORM_REGISTRY, DEFAULT_PLATFORMS, BASE_PATHS, PLATFORMS_STATE_FILE } = require("../registry/index");
...
function getInstalledPlatforms(cwd) {
  ...
  if (fs.existsSync(stateFile)) {
    try {
      ...
      // Defensive: a hand-edited state file could be a non-array (e.g. `{}`),
      // which would later trigger `installed.includes is not a function` in
      // platform.js. Coerce to array before returning.
...
function installPlatform(ctx, key) {
  ...
  p.files.forEach(({ src, dest, merge }) => {
    ...
    if (fs.existsSync(destPath)) {
      if (merge && src.endsWith(".json") && fs.existsSync(srcPath)) {
        // P-PI-001 (M-017): Merge JSON config - union arrays, preserve existing
        // scalar fields. Ensures user's existing packages/settings are kept
        // while template skills/prompts fields are added.
        try {
          ...
          console.log(`  ✅ ${isZh ? "已合并" : "Merged"} ${dest}`);
        } catch (err) {
          console.log(`  ℹ️  ${dest} ${isZh ? "已存在，跳过" : "already exists. Skipping"}（merge: ${err.message}）`);
        }
      } else {
...
function removePlatform(ctx, key) {
  ...
  p.links.forEach(({ link }) => {
    ...
    try {
      ...
      // Use unlinkSync for symlinks: rmSync can silently no-op on broken
      // symlinks (target missing), leaving the dangling link behind.
