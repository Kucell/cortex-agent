...
// D-ARI-P007-host-completeness revision 4c50a96c..., per P-007 §4.6):
// extends the existing four-host whitelist with `dsh` / `codey` / `minimax`
// in stable order. VC-031-002-03 — "SUPPORTED_HOSTS contains cursor, dsh,
// codey, and minimax without regressing the existing four hosts".
// No second-truth whitelist file is introduced; this array IS the single
// source of truth for `--host` argument validation (lib/dispatch/cli.js +
// executeDispatch ERR_HOST_INVALID). The dispatcher continues to fail
// closed for any host outside this list.
const SUPPORTED_HOSTS = Object.freeze([
  "claude-code",
  "pi",
  "codex",
  "cursor",
  "dsh",      // P-007 / M-029 first-class dispatch host
  "codey",    // P-007 / M-003 minimax sibling
  "minimax",  // P-007 / M-003 minimax (the original "MiniMax" branded host)
]);
...
// MS-003: Get idempotency directory using shared runtime-layout API (VC-011)
...
function idempotencyDir(projectRoot) {
  ...
  if (paths.legacyExists && !paths.activated) {
    return path.join(paths.dispatch.legacy, "idempotency");
  }
  return path.join(paths.dispatch.new, "idempotency");
}
