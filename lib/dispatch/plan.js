...
const dispatchState = require("../coordination/dispatch-state.js");
const { matchExecutionSurface, ExecutionSurfaceError } = require("../runtime-adapters/execution-surface-matcher.js");
...
function summarizeLeases(root) {
  ...
  const leasesDir = paths.legacyExists && !paths.activated
    ? path.join(paths.coordination.legacy, "leases")
    : path.join(paths.coordination.new, "leases");
