...
class ExternalMcpClient {
  ...
  close() {
    // Returns a Promise that resolves when the child subprocess has
    // actually exited. This is critical for test teardown: a live
    // child with open stdio handles keeps the libuv event loop
    // alive, which makes node --test hang. Callers should `await
    // client.close()` to make sure the child is fully reaped before
    // the test returns.
    ...
    if (this._child) {
      ...
      if (sigtermTimer.unref) sigtermTimer.unref();
    }
...
class McpBridge {
  ...
  // SIGTERMs the child. Returns a Promise that resolves when the client
  // subprocess (if we own it) has actually exited, so callers can await
  // before the test returns and avoid leaving a zombie child holding
  // inherited stdio handles open.
  async close() {
    let clientPromise = Promise.resolve();
    if (this.client && this._ownsClient) {
      try {
        clientPromise = Promise.resolve(this.client.close());
      } catch (_) { /* ignore */ }
    }
