...
const {
  ...
  return {
    VALID_ROLES: require("./registry").VALID_ROLES,
    VALID_STATUSES: require("./registry").VALID_STATUSES,
    VALID_ADAPTER_TYPES: require("./registry").VALID_ADAPTER_TYPES,
    isValidRole: (r) => require("./registry").VALID_ROLES.includes(r),
    isValidStatus: (s) => require("./registry").VALID_STATUSES.includes(s),
    isValidAdapterType: (t) => require("./registry").VALID_ADAPTER_TYPES.includes(t),
  };
})();
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg && arg.startsWith("--agent=")) {
      out.projectFlag = arg.slice("--agent=".length);
      continue;
    }
    if (arg === "--add") {
      out.add = true;
...
function printHelp() {
  ...
  cortex-agent agent projects set <agent_id> <project_id> [--add|--remove]
  cortex-agent agent projects list [--agent <agent_id>]
  cortex-agent agent projects members <project_id>
...
Governed Pi launch:
  cortex-agent agent launch pi --task-id <id> --agent-id <id> --session-id <id> \\
    --lease-id <id> --fencing-token <n> --worktree <absolute-path> --task-file <file>
  Pi is discovered from PATH; --command, --allow-command, --agent-arg and command overrides are rejected.
...
// ─── subcommand: projects (P-002) ───────────────────────────────────────────

function runProjects(parsed, lang) {
  ...
  const failUsage = (msg) => {
    const text = lang === "zh" ? `用法错误: ${msg}` : `Usage error: ${msg}`;
    process.stderr.write(`${text}\n`);
    process.exitCode = 2;
  };

  if (!op) {
    failUsage("projects requires an operation: set | list | members");
    return;
  }

  if (op === "set") {
    const agentId = positional[2];
    const projectId = positional[3];
    if (!agentId || !projectId) {
      failUsage("projects set <agent_id> <project_id> [--add|--remove]");
      return;
    }
    if (parsed.add && parsed.remove) {
      failUsage("pass --add OR --remove, not both");
      return;
    }
    try {
      const updated = registry.setAgentProjects(root, agentId, projectId, {
        add: !parsed.remove,
        remove: parsed.remove,
      });
      if (parsed.outputJson) {
        printJson({ ok: true, agent_id: agentId, projects: registry.entryProjects(updated) });
      } else {
        const opName = parsed.remove ? "removed from" : "added to";
        process.stdout.write(`✅ project \"${projectId}\" ${opName} agent \"${agentId}\" (projects: ${registry.entryProjects(updated).join(", ") || "(none)"})\n`);
      }
    } catch (error) {
      process.stderr.write(`${error.message}\n`);
      process.exitCode = error.code === "ERR_AGENT_NOT_FOUND" ? 3 : 2;
    }
    return;
  }
  ...
  if (op === "list") {
    try {
      if (parsed.projectFlag) {
        const projects = registry.agentProjects(root, parsed.projectFlag);
        if (projects === null) {
          failUsage(`agent \"${parsed.projectFlag}\" not found`);
          return;
        }
        if (parsed.outputJson) {
          printJson({ agent_id: parsed.projectFlag, projects });
        } else {
          process.stdout.write(`agent \"${parsed.projectFlag}\": ${projects.join(", ") || "(none)"}\n`);
        }
      } else {
        const rows = registry.listAgents(root).map((entry) => ({
          agent_id: entry.agent_id,
          projects: registry.entryProjects(entry),
        }));
        if (parsed.outputJson) {
          printJson({ agents: rows });
        } else {
          for (const row of rows) {
            process.stdout.write(`- ${row.agent_id}: ${row.projects.join(", ") || "(none)"}\n`);
          }
        }
      }
    } catch (error) {
  ...
  if (op === "members") {
    ...
    if (!projectId) {
      failUsage("projects members <project_id>");
      return;
    }
    try {
      const members = registry.projectMembers(root, projectId);
      if (parsed.outputJson) {
        printJson({ project_id: projectId, agents: members.map((e) => e.agent_id) });
      } else {
        process.stdout.write(`project \"${projectId}\": ${members.map((e) => e.agent_id).join(", ") || "(none)"}\n`);
      }
    } catch (error) {
      process.stderr.write(`${error.message}\n`);
      process.exitCode = 2;
  ...
  failUsage(`unknown projects operation \"${op}\" (set|list|members)`);
}
...
function agentRegistryCommand(ctx) {
  ...
  const args = rawArgs[0] === "agent" || rawArgs[0] === "agents" ? rawArgs.slice(1) : rawArgs;
  ...
  if (parsed.subcommand === "projects") {
    return runProjects(parsed, ctx.lang);
  }
  ...
  process.stderr.write(
    ...
    `Valid: discover, invoke, projects (M-002 Agent Registry) | report, launch (M-008 coordination runtime).\n`,
  );
  process.exitCode = 2;
}
