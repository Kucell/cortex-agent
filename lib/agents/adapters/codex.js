...
// ─── Codex Adapter (M-003 MS-002 / F-003) ───────────────────────────────────────
//
// Concrete adapter for the OpenAI Codex CLI. Mirrors the M-003 MS-001
// claude-code adapter (lib/agents/adapters/claude-code.js) one-for-one so the
// framework treats every external vendor uniformly. The 5-method contract
// (discover / health / invoke / cancel / report) is implemented in
// BaseAdapter; this file provides the Codex-specific bits:
//
//   - Codex CLI invocation shape: `codex --json --run-id <id> --task <task>
//     [--input <input>] [--config <configRef>]` (same argv shape as
//     claude-code so dispatch envelopes are interoperable; the upstream
//     Codex CLI is a Node-runnable binary that defaults to plain JSON or
//     Content-Length framed JSON-RPC over stdio).
//   - subprocess isolation: tests inject a fake `codex` binary path (no
//     real Codex CLI in the test environment).
//   - failure modes:
...
//
// Hard constraints (per VC-M-003-MS-002-codex §architecture_hard_constraints):
//   - Zero npm deps. node:child_process / node:fs / node:path only.
//   - No real Codex CLI call in tests — tests inject a fake binary path.
//   - Atomic journal writes (writeDispatchArtifact does .tmp + rename).
//   - Pure add: do not modify base.js / claude-code.js / index.js / etc.
...
class CodexAdapter extends BaseAdapter {
  constructor(options = {}) {
    super(options);
    this.bin = options.bin || process.env.CODEX_BIN || DEFAULT_BIN;
  ...
  // 2. health — verify the codex binary is reachable. Uses `which` (POSIX)
  //    or `where` (Windows). Cheap, safe to call from CLI / dashboard. Mirrors
  //    the claude-code health check shape so `agent adapter health` output
  //    stays consistent across vendors.
  async health() {
  ...
  // 3. invoke — execute a real Codex dispatch.
  //    payload: { task, input, ... }
    ...
    if (finalState.code !== 0) {
      return this._writeErrorAndRollback(projectRoot, runId, {
        ...
        error: {
          ...
          message: `codex exited with code ${finalState.code}`,
      ...
      return this._writeErrorAndRollback(projectRoot, runId, {
        ...
        error: {
          code: parsed.error.code
            ? `ERR_CODEX_${String(parsed.error.code).toUpperCase()}`
            : "ERR_CODEX_RPC_ERROR",
          message: parsed.error.message || "Codex returned a JSON-RPC error",
  ...
  _parseJsonRpc(stdoutData) {
    ...
    // Try plain JSON first (most common — codex --json emits single-line JSON).
    try {
      return JSON.parse(trimmed);
    } catch (_) { /* fall through to framed parse */ }
    // Try Content-Length framed JSON-RPC (per JSON-RPC over stdio spec; the
    // Codex app-server mode also uses this framing for streaming responses).
  ...
  _writeErrorAndRollback(projectRoot, runId, errorRecord) {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `[codex] critical: failed to write any error/rollback for ${runId}: ${finalErr.message}`,
        );
      }
    }
    void written;
...
// ─── registration note (M-003 MS-002 / F-003) ────────────────────────────────
//
// This module does NOT self-register on require (unlike the MS-001 claude-
// code design where the registry is responsible for seeding). The reason is
// that a direct `require("./index")` from this file creates a circular
// dependency — the registry's `module.exports` is still being built when
// codex.js is being loaded — and Node returns a partial export object
// without `register`, leading to silent registration failure.
//
// Instead, the registry (lib/agents/adapters/index.js) explicitly imports
// this file and calls `register("codex", CodexAdapter)` once its
// initialization is complete. This is documented in the handoff as a minor
// additive change to index.js (a strictly additive block at the bottom of
// the file: 5 lines, no existing statement modified). 3rd party adapter
// authors should follow the SAME pattern: import the registry and call
// `register()` from an entry point that owns the lifecycle, OR add a
// strictly additive `require + register` line to a seed file.
