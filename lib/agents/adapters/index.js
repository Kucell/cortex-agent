...
function _seed() {
  ...
  // M-003 MS-002 / F-003: Codex baseline. We register codex inside _seed()
  // (not just at module-load time) so `reset()` re-establishes the full
  // post-MS-002 baseline. The try/catch is strictly additive — if codex.js
  // is missing (e.g. partial checkout) the seed still serves claude-code.
  // See handoff §deviations for the full rationale.
  try {
    const { CodexAdapter: _CodexAdapter } = require("./codex");
    _REGISTRY.set("codex", _CodexAdapter);
  } catch (_) { /* noop — codex is an optional MS-002 adapter */ }
  // M-029 / P-006: DSH first-class dispatch adapter (M-029 MS-001 shipped
  // the implementation as commit `7d877a8`). Registered via try/catch so a
  // missing dsh.js (e.g. partial checkout) does not break the rest of the
  // adapter surface; failure is observable only when `cortex-agent agent
  // adapter list` is run and `dsh` is absent. See `.agent/missions/M-029/
  // mission-plan.md` §Features F-015 and `lib/agents/adapters/dsh.js` for
  // the adapter surface; the standalone bootstrap path lives at
  // `lib/agents/adapters/dsh-bootstrap.js` (M-029 MS-002 follow-up).
  try {
    const { DshAdapter: _DshAdapter } = require("./dsh");
    _REGISTRY.set("dsh", _DshAdapter);
  } catch (_) { /* noop — dsh is an optional first-class M-029 adapter */ }
  // M-031 MS-001 (P-007 host adapter completeness round, approved
  // D-ARI-P007-host-completeness revision 4c50a96c...): registers
  // observer-mode cursor adapter so `cortex-agent agent adapter list`
  // shows cursor with `observer: true`. See
  // `lib/agents/adapters/cursor.js` for the adapter surface; the
  // standalone bootstrap path lives at
  // `lib/agents/adapters/cursor-bootstrap.js` for opt-in minimal-runtime
  // deployments. Cursor is registered here WITHOUT touching
  // `lib/agents/registry.js` (M-002 frozen) — instead `cursor` is added
  // to VALID_ADAPTER_TYPES_EXT in `lib/agents/registry-adapter-types.js`
  // (commit of this M-031 MS-001 change).
  try {
    const { CursorAdapter: _CursorAdapter } = require("./cursor");
    _REGISTRY.set("cursor", _CursorAdapter);
  } catch (_) { /* noop — cursor is an optional observer-mode M-031 adapter */ }
}
...
// augment; reset() reverts to this seed (which now includes MS-002 codex
// per the F-003 _seed() block above).
_seed();
