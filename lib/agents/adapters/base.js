...
// The dispatch journal lives at .agent-runtime/dispatch/<run_id>/ — distinct
// from M-002's .agent/runs/<run_id>/. Per D-002-3 the path boundary is strict:
// M-002 registry = .agent/agents/ + .agent/runs/ ; M-003 adapters =
// .agent-runtime/dispatch/ . Coordination runtime (M-008) writes
// .agent-runtime/coordination/ — also distinct.
function dispatchDir(projectRoot, runId) {
  ...
  return path.join(projectRoot, ".agent-runtime", "dispatch", runId);
}
