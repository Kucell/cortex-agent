...
- **需求确认前的视觉锚点**：在正式架构设计之前，用流程图和动效 HTML 让团队对齐核心用户路径。
- **UI-heavy 项目早期阶段**：快速生成 Pixso 设计帧，替代手写 Figma/线框图，直接进入交互讨论。
- **轻量流程验证**：仅用 `--mode doc --fidelity low` 输出 Mermaid 图，梳理复杂业务流程而不引入额外工具依赖。
- **需求→实现衔接**：输出的 `validation-contract.json` 可直接作为 `/ship` 交付阶段的验收基准。
...
如果 `.agent/prd/` 或 `.agent/prds/` 存在相关 PRD，优先读取：
...
> **Pixso MCP 不可用时的降级处理**：若 Pixso MCP 工具不可访问（连接超时、未安装、权限不足），自动 fallback 到 `doc` 模式继续执行——跳过 UI_PROTOTYPE 阶段，仅生成 `flow.md` 与（按 fidelity）`prototype.html`。DONE 阶段在摘要中注记：`ui path skipped: MCP unavailable`，并在 `validation-contract.json` 中省略 `runtime` 断言。
...
如果存在关联 PRD，应把生成的原型路径或 Pixso 帧信息回填到 PRD 的 `links.json` 或在 DONE 摘要中提示人工执行 `/prd design <prd-id>` 更新设计状态。
