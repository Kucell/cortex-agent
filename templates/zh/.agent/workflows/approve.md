...
用户不必输入斜杠命令。当前交互中出现明确、无条件的自然语言选择时，主 Agent 应将其路由为 Decision 模式并持久化，而不是把聊天文本本身当作审批证据。

- 明确批准：`批准`、`同意`、`批准并继续`、`approve`、`yes, approve`。
- 明确拒绝：`拒绝`、`不同意`、`reject`。
- 明确要求修改：`需要修改`、`退回修改`、`revise`。
- 不构成选择：`看起来不错`、`可以考虑`、`差不多`、`先看看`、沉默、表情或历史偏好。

路由约束：

1. 用户点名 Decision ID 时，只解析该 ID。
2. 未点名 ID 时，只能在当前提示明确展示了一个 Decision，或系统仅有一个与当前 blocking Waitpoint 关联的 open Decision 时解析。
3. 存在多个候选、选择带条件、资源已变化或语义含糊时，列出 ID、action 和 resource，要求用户明确选择，不得猜测。
4. 把原始用户选择或其忠实摘要写入非空 `rationale`，以 `interactive-user` 或平台提供的稳定用户标识作为 `resolved-by`。
5. 解析成功后，将控制权交还 Decision 的 owning workflow。owner 必须重新计算 resource、验证 Decision 并自行释放 Waitpoint；随后可自动继续已批准范围内的普通步骤。
6. 遇到新的 architecture、merge、destructive、credential、external-side-effect Decision 时再次暂停，不得把一次批准扩展到后续资源。
...
4. 用户必须在当前交互中通过斜杠命令或符合“自然语言入口”的表达明确选择 `approve`、`reject` 或 `revise`。Dashboard 和只读查询只能展示或请求处理，不能视为批准。
