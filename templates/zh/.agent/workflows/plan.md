...
/plan
/plan "实现用户登录功能"
/plan --from-proposal <提案路径>   （读取真实提案文件，推荐）
...
2. 若 `--from-proposal <路径>`：读取指定提案文件（`.agent/plans/proposals/xxx-proposal.md`）
...
2. 在 **活跃任务表** 中追加新任务行（进度默认 0%）；若来自提案，每行带 `Proposal: <路径>` 引用
3. 更新文件头部的 `最后更新` 日期

### 第四步：回填提案（若来自 --from-proposal）

若本次 `/plan` 由 `/approve` 调度（或用户手动传入 `--from-proposal`），
在提案文件头部回填执行载体与状态：
