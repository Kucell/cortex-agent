...
description: 通用 fallback domain —— 当 general 模式项目没有更具体的 domain 匹配时(dialogue / knowledge / content / operations),自动激活本 domain。激活 4 个 general workflow + memory-curator sub-agent,domain 专属配置最小化。
mode: general
schema_version: 1
language: zh
priority: 100  # 数字越大 = 优先级越低;specialized domain 优先于本 fallback
workflows_enabled: [memory-recall, memory-distill, agent-invoke, agent-discover]
subagents_enabled: [memory-curator]
---

# Domain: General-Purpose(general 模式通用占位,中文版)
...
- 英文版: `domains/general-purpose.md`
