...
const { extractMarkdownRefs, parseTaskProgress } = require(path.join(__dirname, "../../../tasks/scripts/task-state.js"));
...
function enrichTaskPreviewRefs(task, fallback) {
  const explicit = Array.isArray(task?.source_refs) ? task.source_refs : [];
  const fallbackRefs = Array.isArray(fallback?.source_refs) ? fallback.source_refs : [];
  const extracted = extractMarkdownRefs(
    task?.title,
    task?.plan,
    task?.description,
    task?.progress,
  ...
  const sourceRefs = [...new Set([...explicit, ...extracted, ...fallbackRefs])];
  return {
    ...task,
...
function main() {
  ...
  const tasks = (managed?.tasks || parsedTasks).map((task) => enrichTaskPreviewRefs(task, parsedById.get(task.id)));
...
function loadPreview(path) {
  const body = document.getElementById('preview-body');
  const overview = document.getElementById('overview-text');
  if (body) body.textContent = 'Loading ' + path + '...';
  return fetch('/api/preview?path=' + encodeURIComponent(path))
    ...
    .catch(function (error) {
      if (overview) overview.textContent = '无法读取关联内容';
