...
let queryGovernedAttemptProgress = null;
let queryGovernedAttemptDiagnostics = null;
try {
  ({
    queryGovernedAttemptProgress,
    queryGovernedAttemptDiagnostics,
  } = require("./query-governed-attempt.js"));
} catch (_) {
  // Capability discovery must omit unavailable handlers (M-013 SP-006 additive).
}
...
function governedAttemptProgressProjection() {
  if (!queryGovernedAttemptProgress) {
    fail("governed_attempt_projection_unavailable", "governed-attempt-progress projection module is missing.", 2);
  }
  return dispatchEnvelope("governed-attempt-progress", queryGovernedAttemptProgress(agentRoot));
}

function governedAttemptDiagnosticsProjection() {
  if (!queryGovernedAttemptDiagnostics) {
    fail("governed_attempt_diagnostics_unavailable", "governed-attempt-diagnostics projection module is missing.", 2);
  }
  return dispatchEnvelope("governed-attempt-diagnostics", queryGovernedAttemptDiagnostics(agentRoot));
}
