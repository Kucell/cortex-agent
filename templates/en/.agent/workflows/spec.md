...
<!-- EN translation pending: structural English skeleton only; the detailed Chinese body below is retained verbatim as the source of truth. TODO: translate Purpose, Status Machine notes, Step-by-Step details, Non-Goals, Verification, and Cross-References into English. -->

# Lightweight Spec Workflow (/spec)

## Usage

```text
/spec start "<spec-id>" "<initial requirement description>"
/spec status [<spec-id>]
...
> Invocation: a vague verbal/typed request → the AI suggests `/spec start "<id>" "<requirement>"` and enters CLARIFY.
...
No Task Pipeline gate; no `.agent/tasks/<id>.json` record is created. Upgrade to `/prd` / `/plan` / `/mission` when long-term tracking is needed.

## Boundaries vs Adjacent Workflows

| Workflow | 适用场景 | 升级路径（来自 /spec） |
| :--- | :--- | :--- |
