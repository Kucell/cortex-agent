...
`/approve` is one of the **only legitimate write entries** for Decisions in the Communication Runtime. Every resource approval (proposal, merge, release, destructive action, external side effect) must pass through the decisions / waitpoints gates:

- **Resource digest rule**: every approval must record a resource digest in the form `#digest:<resource-digest>` (`merge:<task-id>@<digest>`, `release:<project>@<candidate_digest>`, `proposal:<proposal-path>@<file-digest>`, `destructive:<action>@<digest>`). Digests must come from `git rev-parse` / `sha256sum` or schema fields — never fabricated.
- **Pre-approval request (path A/B)**: the upstream workflow (`/arch-design` or `/mission`) must first `decisions request --gate mission --type approval --gate-action architecture --resource-ref proposal:<path>@<digest>` to create an open Decision, then immediately `waitpoints create --owner-workflow /approve --reason "Approval required" --action architecture --resource-ref proposal:<path>@<digest>`.
- **User approval**: use `decisions resolve --gate user --status approved --resolved-by interactive-user --rationale "<user's words or a one-sentence reason>"`. **`/approve` MUST NOT call `waitpoints release`** — the Waitpoint is released by the owning workflow (`/plan` or `/mission`) after resolution.
- **Multi-choice scenarios**: when the user picks among several open Decisions, the main Agent must first `query decisions` to list candidates, confirm the user explicitly named a Decision ID, then write. Ambiguous expressions ("looks fine", "any of them works") require a follow-up question.
- **Reject / revision**: `decisions resolve --status rejected|revision_requested` auto-cancels the matching Waitpoint; the owning workflow must stop the current path and revert to draft.
- **Silence is not approval**: if the user does not reply, or replies only with an emoji / punctuation, ask again — never default to approved.
- **One approval does not extend to subsequent resources**: new architecture, merge, release, destructive, credential, or external-side-effect Decisions each require their own pause for confirmation.
