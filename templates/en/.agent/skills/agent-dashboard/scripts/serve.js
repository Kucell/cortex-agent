...
const port = Number(arg("--port", process.env.AGENT_DASHBOARD_PORT || "8787"));
...
function generate() {
  ...
  return { ok: result.status === 0, stdout: result.stdout, stderr: result.stderr, at: new Date().toISOString() };
}
...
function withLiveReload(html) {
  const snippet = `<script>(()=>{const s=new EventSource('/events');s.onmessage=(e)=>{try{const p=JSON.parse(e.data);if(p.type==='reload')location.reload();}catch(_){}};})();</script>`;
...
const server = http.createServer((req, res) => {
  ...
  const html = fs.existsSync(outPath) ? fs.readFileSync(outPath, "utf8") : `<pre>${last.stderr || "Dashboard has not been generated yet."}</pre>`;
  res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
  res.end(withLiveReload(html));
});

server.listen(port, "127.0.0.1", () => {
  console.log(JSON.stringify({ ok: true, url: `http://127.0.0.1:${port}`, output: path.relative(root, outPath), interval_ms: intervalMs }, null, 2));
});
