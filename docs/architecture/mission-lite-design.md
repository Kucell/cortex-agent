...
推荐采用 Mission Lite，但按“验证契约优先”的顺序落地。

优先级理由：

1. `validation-contract` 是最小核心闭环，先落地它就能改善 `/start-task` 和 `/ship`。
2. `/mission` workflow 已作为模板级工作流接入，可用于长周期任务的手动编排。
3. planner / reviewer 的 JSON 契约扩展已保持向后兼容；旧 `plan_summary` 和 `review_verdict` 字段不删除。

Mission Lite 第一版模板级闭环已完成。下一步如继续增强，应进入自动化或运行时证据集成，而不是继续扩张基础概念。
