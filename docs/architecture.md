...
## 六、Runtime Continuity 跨工具会话连续性

Runtime Continuity 是 Cortex Agent 面向同一项目、不同 agent 工具切换的恢复层。它不读取 Claude Code、Codex、Cursor 等宿主的私有完整对话，而是要求 agent 通过标准 CLI 把可迁移工作日志、阶段断点和恢复快照写入 `.agent/runtime-continuity/`。

详细设计见 [Runtime Continuity v2 设计](architecture/runtime-continuity-v2-design.md)。

| 能力 | 说明 |
| :--- | :--- |
| 工作日志同步 | `runtime-continuity log/checkpoint` 记录目标、进度、命令、文件和下一步 |
| 长会话存档 | `archive --full` 同时写入项目内 JSON archive 与 `~/.agent/contexts/<project>/latest.md` |
| 自动接手 | `resume-bundle` 汇总 latest archive、handoff、run、session、artifact 和 git 状态 |
| 跨宿主切换 | `host-switch` 创建结构化 archive，并记录 source/target host |

Runtime Continuity 与 `/handoff` 的边界：前者负责“会话和宿主工具如何恢复”，后者负责“具体任务如何移交给下一个 agent”。长任务应同时使用 Mission Lite 的 mission state、`/handoff` 的 task transfer，以及 Runtime Continuity 的 session archive。
