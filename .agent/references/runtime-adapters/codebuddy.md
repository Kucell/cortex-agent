...
腾讯 CodeBuddy agent(open-design 上游支持 `od mcp install codebuddy`)。cortex-agent 侧契约已文档化。

## 2. Installation

```bash
# 一键安装(MCP server)
manual — see codebuddy.md
# open-design 上游安装
od mcp install codebuddy
```
...
配置文件: unknown — CodeBuddy(腾讯)的 MCP config 路径未公开,安装为 manual。
