...
const en = path.join(root, "templates/en/.agent/workspaces/scripts/worktree-layout.js");
const zh = path.join(root, "templates/zh/.agent/workspaces/scripts/worktree-layout.js");
...
test("English and Chinese machine scripts are byte-identical", () => {
  assert.deepEqual(fs.readFileSync(en), fs.readFileSync(zh));
});
...
test("default layout uses one sibling repository container", () => {
  ...
  fs.mkdirSync(repo);
  git(repo, ["init", "-q"]);
  const value = run(en, ["resolve", "--repo", repo, "--task-id", "T-001-auth"]);
...
test("legacy repository prefix is removed from child name", () => {
  ...
  fs.mkdirSync(repo);
  git(repo, ["init", "-q"]);
  const value = run(en, ["resolve", "--repo", repo, "--name", "Demo-M007"]);
...
test("repository-internal root fails closed without explicit policy", () => {
  ...
  fs.mkdirSync(repo);
  git(repo, ["init", "-q"]);
  assert.throws(() => cp.execFileSync(process.execPath, [en, "resolve", "--repo", repo, "--root", path.join(repo, ".worktrees"), "--name", "T-1"]));
});
...
test("migration plan is read-only and requires a full status/process audit", () => {
  ...
  const value = run(en, ["plan", "--repo", repo]);
